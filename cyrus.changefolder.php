...
function default_directory_size(){
	$sock=new sockets();
	echo $sock->getFrameWork("cmd.php?DirectorySize=".base64_encode("/var/spool/cyrus/mail"));
}
function current_directory_size(){
	$sock=new sockets();
	$directorypath=base64_decode($sock->getFrameWork("cmd.php?cyrus-get-partition-default=yes"));	
	echo $sock->getFrameWork("cmd.php?DirectorySize=".base64_encode("$directorypath"));
}
function CyrusMoveToCurrent(){
