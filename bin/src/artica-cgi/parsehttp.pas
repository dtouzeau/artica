...
RegExpr.expression:='vgdisplay:(.+)';
if RegExpr.Exec(uri) then begin
   tmpstr:=logs.FILE_TEMP();
   cmd:=SYS.LOCATE_LVDISPLAY()+ ' -v -m '+RegExpr.Match[1]+' >'+tmpstr+' 2>&1';
   logs.Debuglogs(cmd);
   fpsystem(cmd);
   FileData.Add(logs.ReadFromFile(tmpstr));
   logs.DeleteFile(tmpstr);
...
//-----------------------------------------------------------------------------------
RegExpr.expression:='^MoveLDAPOu:(.+?);(.+)';
if RegExpr.Exec(uri) then begin
   tmpstr:=logs.FILE_TEMP();
   cmd:=SYS.LOCATE_PHP5_BIN()+' /usr/share/artica-postfix/exec.ldap.move-orgs.php '+RegExpr.Match[1]+' '+RegExpr.Match[2];
   logs.Debuglogs('MoveLDAPOu:"'+cmd+'"');
   fpsystem(SYS.LOCATE_PHP5_BIN()+' /usr/share/artica-postfix/exec.ldap.move-orgs.php '+RegExpr.Match[1]+' '+RegExpr.Match[2] +' >'+tmpstr+' 2>&1');
   FileData.Add(logs.ReadFromFile(tmpstr));
   logs.DeleteFile(tmpstr);
   exit(true);
end;
...
RegExpr.expression:='^CompileProxy';
  if RegExpr.Exec(uri) then begin
     SYS.COMPILE_PROXY();
     exit(true);
  end;
...
RegExpr.expression:='^BackGroundDCHPD';
  if RegExpr.Exec(uri) then begin
      GLOBAL_INI.THREAD_COMMAND_SET(SYS.LOCATE_PHP5_BIN()+' /usr/share/artica-postfix/exec.dhcpd.compile.php');
      exit(true);
  end;

RegExpr.expression:='^BackGroundBIND9';
  if RegExpr.Exec(uri) then begin
      GLOBAL_INI.THREAD_COMMAND_SET(SYS.LOCATE_PHP5_BIN()+' /usr/share/artica-postfix/exec.dhcpd.compile.php --bind');
      exit(true);
  end;
...
RegExpr.expression:='^MailBoxRemoteSync:(.+)';
  if RegExpr.Exec(uri) then begin
   ...
   if RegExpr.Exec(uri) then begin
      if RegExpr.Match[1]='APP_ROUNDCUBE' then begin
         logs.INSTALL_STATUS('APP_ROUNDCUBE',3);
         GLOBAL_INI.THREAD_COMMAND_SET(GLOBAL_INI.get_ARTICA_PHP_PATH()+'/bin/artica-roundcube --install');
         exit(true);
      end;
       tempstr:=GLOBAL_INI.get_ARTICA_PHP_PATH()+'/ressources/install/' +RegExpr.Match[1] + '.dbg';
       logs.Debuglogs('Set the log file in' + tempstr);
       logs.INSTALL_STATUS(RegExpr.Match[1],3);
       logs.INSTALL_PROGRESS(RegExpr.Match[1],'{scheduled}');
       GLOBAL_INI.THREAD_COMMAND_SET(GLOBAL_INI.get_ARTICA_PHP_PATH()+'/bin/artica-make '+RegExpr.Match[1] +' >'+tempstr +' 2>&1');
       exit(true);
   end;
...
RegExpr.expression:='aptcheck';
   if RegExpr.Exec(uri) then begin
      tmpstr:='/etc/artica-postfix/apt.upgrade.cache';
      if not FileExists(tmpstr) then begin
           logs.Debuglogs('aptcheck:: unable to stat ' + tmpstr);
           exit(true);
       end;
       FileData.LoadFromFile(tmpstr);
       logs.Debuglogs('aptcheck:: ' + IntTostr(FileData.Count) + ' lines');
       exit(true);
   end;
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------
 RegExpr.expression:='GetLocalSid';
   if RegExpr.Exec(uri) then begin
      FileData.Add(GLOBAL_INI.SYSTEM_LOCAL_SID());
      exit(true);
   end;
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 RegExpr.expression:='stunnel4save';
   if RegExpr.Exec(uri) then begin
      logs.OutputCmd('/etc/init.d/artica-postfix restart stunnel');
      FileData.Add('{success}');
      exit(true);
   end;

 RegExpr.expression:='stunnel4status';
   if RegExpr.Exec(uri) then begin
      FileData.Add(stunnel.STUNNEL_STATUS());
      exit(true);
   end;
//----------------------------------------------------------------------------------------------------------------------------------------------------------------
...
// SpamAssassin ########################################################################################################################################
   RegExpr.expression:='MILTER_SPAMASS_STATUS';
   if RegExpr.Exec(uri) then begin
         FileData.Add(spamass.MILTER_STATUS());
         exit(true);
   end;
...
// CYRUS CYRUS CYRUS ===========================================================
    ...
    RegExpr.expression:='^pattern_status_generate';
     if RegExpr.Exec(uri) then begin
        FileData.Add(GLOBAL_INI.STATUS_PATTERN_DATABASES());
        exit(true);
     end;
...
RegExpr.expression:='obmapachestatus';
   ...
   RegExpr.expression:='clamav_status';
     if RegExpr.Exec(uri) then begin
       FileData.Add(clamav.CLAMAV_STATUS()+clamav.MILTER_STATUS()+clamav.FRESHCLAM_STATUS());
        logs.logs('ParseUri:: clamav_status status');
        exit(true);
    end;
...
RegExpr.expression:='squidclient:(.+)';
     if RegExpr.Exec(uri) then begin
        logs.logs('ParseUri:: squid client action:'+ RegExpr.Match[1]);
        if not FileExists(squid.SQUIDCLIENT_BIN_PATH()) then begin
           FileData.Add('Unable to stat squidclient in this system...');
           exit(true);
        end;
        cmd:=squid.SQUIDCLIENT_BIN_PATH() + ' cache_object://127.0.0.1/' + RegExpr.Match[1] + ' >/opt/artica/tmp/squid.tmp 2>&1';
        logs.logs('ParseUri:: squid client action:'+ cmd);
        fpsystem(cmd);
        if FileExists('/opt/artica/tmp/squid.tmp') then begin
           FileData.LoadFromFile('/opt/artica/tmp/squid.tmp');
           logs.DeleteFile('/opt/artica/tmp/squid.tmp');
           exit(true);
        end;
    end;
...
RegExpr.expression:='squid_status';
     if RegExpr.Exec(uri) then begin
        FileData.Add(squid.SQUID_STATUS()+kav4proxy.KAV4PROXY_STATUS()+dansguardian.DANSGUARDIAN_STATUS()+dansguardian.C_ICAP_STATUS());
        logs.logs('ParseUri:: squid status');
        exit(true);
    end;
...
RegExpr.expression:='logs_squid';
   if RegExpr.Exec(uri) then begin
      LOGS.logs(' get squid logs ');
      tempstr:=LOGS.FILE_TEMP();
      cmd:='/usr/bin/tail -n 100 '+squid.SQUID_GET_CONFIG('access_log') +' >'+tempstr;
      fpsystem(cmd);
      LOGS.logs(cmd);
      if FileExists(tempstr) then begin
         FileData.LoadFromFile(tempstr);
...
RegExpr.expression:='usb_mount:(.+?);(.+)';
    ...
    RegExpr.expression:='nicstatus:(.+)';
    if RegExpr.Exec(uri) then begin
       tmpstr:=GLOBAL_INI.SYSTEM_GET_LOCAL_IP(RegExpr.Match[1]);
       tmpstr:=tmpstr+';'+GLOBAL_INI.SYSTEM_GET_LOCAL_MAC(RegExpr.Match[1]);
       tmpstr:=tmpstr+';'+GLOBAL_INI.SYSTEM_GET_LOCAL_MASK(RegExpr.Match[1]);
       tmpstr:=tmpstr+';'+GLOBAL_INI.SYSTEM_GET_LOCAL_BROADCAST(RegExpr.Match[1]);
       tmpstr:=tmpstr+';'+GLOBAL_INI.SYSTEM_GET_LOCAL_GATEWAY(RegExpr.Match[1]);
       if GLOBAL_INI.IsWireless(RegExpr.Match[1]) then tmpstr:=tmpstr+';yes' else tmpstr:=tmpstr+';no';
       if GLOBAL_INI.IsIfaceDown(RegExpr.Match[1]) then tmpstr:=tmpstr+';yes' else tmpstr:=tmpstr+';no';
   ...
   if RegExpr.Exec(uri) then begin
       FileData.Add(GLOBAL_INI.SYSTEM_NETWORK_INFO_NIC(RegExpr.Match[1]));
       result:=true;
       exit;
   end;
...
if RegExpr.Exec(uri) then begin
 ...
 //------------------------------------------------------------------------------
   ...
   RegExpr.expression:='^GetCurrentBindate';
     if RegExpr.Exec(uri) then begin
        tmpstr:=logs.FILE_TEMP();
        fpsystem('/bin/date "+%d-%m %H:%M:%S" >' + tmpstr + ' 2>&1');
        FileData.Add(logs.ReadFromFile(tmpstr));
        logs.DeleteFile(tmpstr);
        logs.Debuglogs('bindate:' + FileData.Text);
        result:=true;
        exit;
    end;
 //------------------------------------------------------------------------------
   RegExpr.expression:='^SetServerTime:(.+)';
     if RegExpr.Exec(uri) then begin
        tmpstr:=logs.FILE_TEMP();
        fpsystem('/bin/date '+ RegExpr.Match[1]+' >' + tmpstr + ' 2>&1');
        FileData.Add(logs.ReadFromFile(tmpstr));
        logs.DeleteFile(tmpstr);
        logs.Debuglogs('SetServerTime:' + FileData.Text);
        result:=true;
        exit;
    end;
 //------------------------------------------------------------------------------
