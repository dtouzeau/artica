...
if not FileExists('/etc/artica-postfix/exec/exec.sh') then begin
  ...
  //if FileExists(artica_path + '/bin/artica-send') then  fpsystem(artica_path + '/bin/artica-send --fix-sql');
  fpsystem(artica_path + '/bin/artica-ldap --maintenance');
...
begin
       ...
       if FileExists('/etc/artica-postfix/crossroads.indentities.conf') then begin
            crossroads_master_name:=GLOBAL_INI.CROSSROADS_MASTERNAME();
            if length(crossroads_master_name)=0 then logs.logs('web_settings():: crossroads_master_name=null !!!, please, check [INFOS],master_name in /etc/artica-postfix/crossroads.indentities.conf' );
            logs.Debuglogs('web_settings() -> replication mode enabled (master name:"' + crossroads_master_name+'"), launching local configurations');
            logs.Debuglogs('web_settings() -> '+php_path + '/bin/artica-ldap -maincf');
            fpsystem(php_path + '/bin/artica-ldap -maincf');
       end;
   ...
   if FIleExists('/etc/artica-postfix/KASPERSKY_WEB_APPLIANCE') then list.Add('$_GLOBAL["KASPERSKY_WEB_APPLIANCE"]=True;') else list.Add('$_GLOBAL["KASPERSKY_WEB_APPLIANCE"]=False;');
   ...
   if FileExists(pdns.BIN_PATH()) then list.Add('$_GLOBAL["POWER_DNS_INSTALLED"]=True;') else list.Add('$_GLOBAL["POWER_DNS_INSTALLED"]=False;');
   pdns.free;
   ...
   if FIleExists('/usr/local/share/artica/group-office/classes/base/config.class.inc.php') then  list.Add('$_GLOBAL["GROUPOFFICE_INSTALLED"]=True;') else list.Add('$_GLOBAL["GROUPOFFICE_INSTALLED"]=False;');
    ...
    if FileExists(SYS.LOCATE_NMAP()) then begin
       ...
       list.Add('$_GLOBAL["nmap_version"]="'+GLOBAL_INI.NMAP_VERSION()+'";');
    end else begin
    ...
    list.Add('$_GLOBAL["disks_size"]="' + SYS.DISKS_STATUS_DEV()+'";');
     ...
     APP_ATOPENMAIL:=trim(GLOBAL_INI.ATMAIL_VERSION());
...
//*********************************** POSTFIX END **********************************
 ...
 if FileExists(clamav.CLAMD_BIN_PATH()) then begin
         ...
         list.Add('$_GLOBAL["CLAMAV_SOCKET"]="' + GLOBAL_INI.CLAMD_GETINFO('LocalSocket')+'";');
...
logs.Debuglogs('web_settings() -> 75%');
 ...
 //-----------------------------------------------------------------------------------------------------
    ...
    if fileExists(awstats.AWSTATS_PATH()) then  begin
    ...
    logs.Debuglogs('web_settings() Checking roundcube...');
    if DirectoryExists(roundcube.web_folder()) then begin
          ...
          list.Add('$_GLOBAL["roundcube_web_folder"]="'+roundcube.web_folder() + '";');
...
begin
   ...
   if SYS.PROCESS_EXIST(pid) then begin
      ...
      if min>25 then begin
         logs.Syslogs('Kill bad keepup2date ghost process :'+pid);
         fpsystem('/bin/kill -9 '+pid);
         logs.NOTIFICATION('[ARTICA]: ('+SYS.HOSTNAME_g()+') keepup2date was killed','The proccess running since '+IntTOStr(min)+' minutes and reach 25 minutes to live','system');
      end;
   end;
...
if FIleExists(cyrus.CYRUS_DAEMON_BIN_PATH()) then begin
   ...
   pid:=SYS.PIDOF_PATTERN('bin/artica-update');
   logs.Debuglogs('CleanBadProcesses():: artica-update ('+pid+')');

     if SYS.PROCESS_EXIST(pid) then begin
      min:=SYS.PROCCESS_TIME_MIN(pid);
      logs.Debuglogs('artica-update process time :'+ IntTOStr(min)+' minutes live');
      if min>45 then begin
         logs.Syslogs('Kill bad artica-updateghost process :'+pid);
         fpsystem('/bin/kill -9 '+pid);
      end;
   end else begin
      logs.Debuglogs('CleanBadProcesses():: artica-update ('+pid+') does not exists in memory');
   end;
