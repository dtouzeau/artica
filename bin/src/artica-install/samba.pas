...
private
     ...
     FUNCTION  SCANNEDONLY_STATUS():string;
     FUNCTION  SMBD_STATUS():string;
     FUNCTION  NMBD_STATUS():string;
     FUNCTION  WINBIND_STATUS():string;
...
begin
  if not FileExists(SMBD_PATH()) then exit;
  result:=SMBD_STATUS()+NMBD_STATUS()+WINBIND_STATUS();
end;
//#########################################################################################
FUNCTION Tsamba.SCANNEDONLY_STATUS():string;
var
   ini:TstringList;
   pid:string;
   smbver:string;
begin
  ...
  ini.Add('');
  ini.Add('[SAMBA_SCANNEDONLY]');
  ini.Add('service_name=APP_SCANNED_ONLY');
  ini.Add('service_cmd=samba');
  ini.Add('service_disabled='+IntToStr(EnableScannedOnly));
  ini.Add('master_version=not applicable');

     if EnableScannedOnly=0 then begin
         result:=ini.Text;
         ini.free;
         SYS.MONIT_DELETE('APP_SCANNED_ONLY');
         exit;
     end;

     if SYS.MONIT_CONFIG('APP_SCANNED_ONLY','/var/run/scannedonly.pid','samba') then begin
         ini.Add('monit=1');
         result:=ini.Text;
         ini.free;
         exit;
...
FUNCTION Tsamba.SMBD_STATUS():string;
var
   ini:TstringList;
   pid:string;
   smbver:string;
begin
  ...
  ini.Add('service_cmd=samba');
  ini.Add('service_disabled='+IntToStr(SambaEnabled));
  ini.Add('remove_cmd=--samba-remove');
  ini.Add('master_version=' + smbver);
  if FileExists('/var/run/smbd.pid') then pid:='/var/run/smbd.pid';
  if FileExists('/var/run/samba/smbd.pid') then pid:='/var/run/samba/smbd.pid';


     if SambaEnabled=0 then begin
         result:=ini.Text;
         ini.free;
         SYS.MONIT_DELETE('APP_SAMBA_SMBD');
         exit;
     end;

      if SYS.MONIT_CONFIG('APP_SAMBA_SMBD',pid,'samba') then begin
         ini.Add('monit=1');
         result:=ini.Text;
         ini.free;
         exit;
...
//#########################################################################################
FUNCTION Tsamba.NMBD_STATUS():string;
var
   ini:TstringList;
   pid:string;
   smbver:string;
begin
  ...
  ini.Add('remove_cmd=--samba-remove');
  ini.Add('master_version=' + smbver);
  if FileExists('/var/run/nmbd.pid') then pid:='/var/run/nmbd.pid';
  if FileExists('/var/run/samba/nmbd.pid') then pid:='/var/run/samba/nmbd.pid';


     if SambaEnabled=0 then begin
         result:=ini.Text;
         ini.free;
         SYS.MONIT_DELETE('APP_SAMBA_NMBD');
         exit;
     end;

      if SYS.MONIT_CONFIG('APP_SAMBA_NMBD',pid,'samba') then begin
         ini.Add('monit=1');
         result:=ini.Text;
         ini.free;
         exit;
...
//#########################################################################################
FUNCTION Tsamba.WINBIND_STATUS():string;
var
   ini:TstringList;
   pid:string;
   smbver:string;
begin
  ...
  if FileExists('/var/run/samba/nmbd.pid') then pid:='/var/run/samba/nmbd.pid';
     ...
     if SambaEnabled=0 then begin
         result:=ini.Text;
         ini.free;
         SYS.MONIT_DELETE('APP_SAMBA_WINBIND');
         exit;
     end;

      if SYS.MONIT_CONFIG('APP_SAMBA_WINBIND','/var/run/samba/winbindd.pid','samba') then begin
         ini.Add('monit=1');
         result:=ini.Text;
         ini.free;
         exit;
