...
private
    ...
    procedure ARTICA_ROBOT_WBL_MASTERCF_SERVICE();
    procedure ARTICA_ROBOT_QUAR_MASTERCF_SERVICE();
    procedure ARTICA_ROBOT_FILTER_MASTERCF_SERVICE();
    procedure ARTICA_ROBOT_SPAM_MASTERCF_SERVICE();
    procedure ARTICA_ROBOT_ZARAFA_SERVICE();
    procedure POSTFIX_ENABLE_SMTP_SASL();
    procedure CLEAN_MASTER_CF();
...
ARTICA_ROBOT_WBL_MASTERCF_SERVICE();

ARTICA_ROBOT_QUAR_MASTERCF_SERVICE();

ARTICA_ROBOT_FILTER_MASTERCF_SERVICE();

ARTICA_ROBOT_SPAM_MASTERCF_SERVICE();
...
begin
    ...
    if not TryStrToInt(SYS.GET_INFO('PostFixSmtpSaslEnable'),PostFixSmtpSaslEnable) then PostFixSmtpSaslEnable:=0;
    if not TryStrToInt(SYS.GET_INFO('PostfixEnableSubmission'),PostfixEnableSubmission) then PostfixEnableSubmission:=0;

    if PostfixEnableSubmission=1 then begin
         logs.Debuglogs('Starting......: POSTFIX Submission is enabled in daemon mode...');
        ENABLE_SUBMISSION(true);
    end else begin
        logs.Debuglogs('Starting......: POSTFIX Submission is disabled in daemon mode...');
     ...
     logs.Debuglogs('Starting......: POSTFIX SASL done...');
end;
//##############################################################################
...
begin
  ...
  list2.Add('mech_list: PLAIN LOGIN');
...
if not trouv then begin
   ...
   try
    ...
    logs.Syslogs('Starting......: Postfix, server artica-whitelist OK in master.cf');
end;
...
if not trouv then begin
   ...
   try
    ...
    logs.Syslogs('Starting......: Postfix, server artica-blacklist OK in master.cf');
end;
...
//##############################################################################
procedure tpostfix.ARTICA_ROBOT_WBL_MASTERCF_SERVICE();
var
   l:Tstringlist;
   RegExpr:TRegExpr;
   i:integer;
...
RegExpr:=TRegExpr.Create;
l.LoadFromFile('/etc/postfix/master.cf');
RegExpr.Expression:='^artica-reportwbl\s+';
...
if not trouv then begin
   logs.Syslogs('Starting......: Postfix, Adding new server artica-reportwbl in master.cf');
   l.Add('');
   l.Add('artica-reportwbl    unix  -       n       n       -       -       pipe');
   l.Add('  flags=F  user=mail argv=/usr/share/artica-postfix/bin/artica-whitelist -a ${nexthop} -s ${sender} --report');
   try
      logs.WriteToFile(l.Text,'/etc/postfix/master.cf');
   except
       logs.Syslogs('ARTICA_ROBOT_WBL_MASTERCF_SERVICE():: warning, unable to write /etc/postfix/master.cf');
   end;
end else begin
    logs.Syslogs('Starting......: Postfix, server artica-reportwbl OK in master.cf');
end;
...
//##############################################################################
procedure tpostfix.ARTICA_ROBOT_ZARAFA_SERVICE();
var
   l:Tstringlist;
   RegExpr:TRegExpr;
   i:integer;
...
if not trouv then begin
   logs.Syslogs('Starting......: Postfix, Adding new server zarafa in master.cf');
   l.Add('');
   l.Add('zarafa    unix  -       n       n       -       -       pipe');
   l.Add('  user=mail argv=/usr/local/bin/zarafa-dagent ${user}');
   try
      logs.WriteToFile(l.Text,'/etc/postfix/master.cf');
   except
       logs.Syslogs('ARTICA_ROBOT_WBL_MASTERCF_SERVICE():: warning, unable to write /etc/postfix/master.cf');
...
procedure tpostfix.ARTICA_ROBOT_QUAR_MASTERCF_SERVICE();
var
   l:Tstringlist;
   RegExpr:TRegExpr;
   i:integer;
...
RegExpr:=TRegExpr.Create;
l.LoadFromFile('/etc/postfix/master.cf');
RegExpr.Expression:='^artica-reportquar\s+';
...
if not trouv then begin
   logs.Syslogs('Starting......: Postfix, Adding new server artica-reportquar in master.cf');
   l.Add('');
   l.Add('artica-reportquar    unix  -       n       n       -       -       pipe');
   l.Add('  flags=F  user=mail argv=/usr/share/artica-postfix/bin/artica-whitelist -a ${nexthop} -s ${sender} --quarantines');
   try
      logs.WriteToFile(l.Text,'/etc/postfix/master.cf');
   except
       logs.Syslogs('ARTICA_ROBOT_WBL_MASTERCF_SERVICE():: warning, unable to write /etc/postfix/master.cf');
    ...
    logs.Syslogs('Starting......: Postfix, server artica-reportquar OK in master.cf');
end;
...
//##############################################################################
procedure tpostfix.ARTICA_ROBOT_SPAM_MASTERCF_SERVICE();
var
   l:Tstringlist;
   RegExpr:TRegExpr;
   i:integer;
...
RegExpr:=TRegExpr.Create;
l.LoadFromFile('/etc/postfix/master.cf');
RegExpr.Expression:='^artica-spam\s+';

forceDirectories('/var/log/artica-whiteblackList');
forceDirectories('/var/spam-mails');
logs.OutputCmd('/bin/chown mail:mail /var/log/artica-whiteblackList');
logs.OutputCmd('/bin/chown mail:mail /var/spam-mails');
...
if not trouv then begin
   logs.Syslogs('Starting......: Postfix, Adding new server artica-reportquar in master.cf');
   l.Add('');
   l.Add('artica-spam    unix  -       n       n       -       -       pipe');
   l.Add('  flags=F  user=mail argv=/usr/share/artica-postfix/bin/artica-whitelist -a ${nexthop} -s ${sender} --spam');
   try
      logs.WriteToFile(l.Text,'/etc/postfix/master.cf');
   except
       logs.Syslogs('ARTICA_ROBOT_SPAM_MASTERCF_SERVICE():: warning, unable to write /etc/postfix/master.cf');
   end;
end else begin
    logs.Syslogs('Starting......: Postfix, server artica-spam OK in master.cf');
end;
...
//##############################################################################
procedure tpostfix.ARTICA_ROBOT_FILTER_MASTERCF_SERVICE();
var
   l:Tstringlist;
   RegExpr:TRegExpr;
   user:string;
...
forceDirectories('/var/mail');
forceDirectories('/var/lib/artica/mail/filter');
logs.OutputCmd('/bin/chown mail:mail /var/mail');

user:=lighttpd.LIGHTTPD_GET_USER();
RegExpr.Expression:='(.+?):(.+)';
if RegExpr.Exec(user) then user:=RegExpr.Match[1];
logs.OutputCmd('/bin/chown -R '+user+' /var/lib/artica/mail/filter');
logs.OutputCmd('/bin/chown -R '+user+' /var/log/artica-filter');

RegExpr.Expression:='^artica-filter\s+';
for i:=0 to l.Count-1 do begin
     if RegExpr.Exec(l.Strings[i]) then begin
        trouv:=true;
     end;
...
if not trouv then begin
   logs.Syslogs('Starting......: Postfix, Adding new server artica-filter in master.cf');
   l.Add('');
   l.Add('artica-filter    unix  -       n       n       -       -       pipe');
   l.Add('  flags=FOh  user='+user+' argv=/usr/share/artica-postfix/exec.artica-filter.php -f ${sender} --  -s ${sender} -r ${recipient} -c ${client_address}');
   try
      logs.WriteToFile(l.Text,'/etc/postfix/master.cf');
   except
       logs.Syslogs('ARTICA_ROBOT_FILTER_MASTERCF_SERVICE():: warning, unable to write /etc/postfix/master.cf');
   end;
end else begin
    logs.Syslogs('Starting......: Postfix, server artica-filter OK in master.cf');
end;
...
//##############################################################################
procedure tpostfix.CLEAN_MASTER_CF();
var
   l:Tstringlist;
   s:Tstringlist;
   i:integer;
...
logs.WriteToFile(s.Text,'/etc/postfix/master.cf');
s.free;
l.free;
end;
//##############################################################################
