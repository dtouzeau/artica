...
l.Add('group daemons');
l.Add('start program = "/etc/init.d/artica-postfix start ldap"');
l.Add('stop program = "/etc/init.d/artica-postfix stop ldap"');
...
begin
   ...
   if FIleExists('/usr/bin/smbpasswd') then begin
       ...
       logs.OutputCmd('/usr/bin/smbpasswd -w '+artica_password);
   end;
...
if FileExists(back_hdb) then begin
  logs.Syslogs('Starting......: OpenLDAP server backen will be hdb');
...
if length(back_hdb)>0 then begin
   l.Add('moduleload'+chr(9)+'back_hdb');
   l.Add('backend'+chr(9)+'hdb');
   l.Add('database'+chr(9)+'hdb');
end else begin
   l.Add('database'+chr(9)+'bdb');
end;
...
l.Add('access to attrs=userPassword,sambaNTPassword,sambaLMPassword,sambaPwdLastSet,sambaPWDMustChange,MailboxSecurityParameters');
