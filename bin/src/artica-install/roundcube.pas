...
SourceCOnf:='/etc/artica-postfix/settings/Daemons/RoundCubeConfigurationFile';
...
begin
  ...
  logs.OutputCmd('/bin/chmod -R 0500 '+webfolder);
  logs.OutputCmd('/bin/chmod 755 '+webfolder+'/temp');
  logs.OutputCmd('/bin/chmod 755 '+webfolder+'/logs');
  if not FileExists(webfolder+'/logs/errors') then logs.OutputCmd('/bin/touch '+webfolder+'/logs/errors');
  logs.OutputCmd('/bin/chmod 755 '+webfolder+'/plugins/artica_ldap_addr/logs');
  ...
  logs.OutputCmd('/bin/chmod 777 '+webfolder+'/logs/errors');
  RegExpr.free;
end;
//##############################################################################
...
if DirectoryExists(folder+'/plugins/sieverules') then begin
   logs.Debuglogs('Starting......: Roundcube replicate plugin sieverules');
   t:=Tstringlist.Create;
   t.add('<?php');
   t.add('$rcmail_config["sieverules_host"] = "127.0.0.1";');
   t.add('$rcmail_config["sieverules_port"] = 2000;');
   ...
   t.add('$rcmail_config["sieverules_include_imap_root"] = FALSE;');
...
if DirectoryExists(folder+'/plugins/managesieve') then begin
   ...
   logs.OutputCmd('/bon/chmod 755 '+folder+'/plugins/managesieve/config.inc.php');
end;
