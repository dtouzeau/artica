...
if Paramstr(1)='--samba-add-computer' then
begin
...
if ParamStr(1)='--squid-remove' then
begin
zsquid:=tsquid.Create();
zsquid.REMOVE();
zkav4proxy:=tkav4proxy.Create(SYS);
...
if ParamStr(1)='--reconfigure-cyrus' then
begin
...
if paramStr(1)='--mastercf-delete-amavis' then
begin
GLOBAL_INI:=myconf.Create;
...
if ParamStr(1)='--cyrus-ctl-cyrusdb' then
begin
...
if ParamStr(1)='--enable-postfix-submission' then
begin
if ParamStr(2)='yes' then
begin
...
if paramStr(1)='--change-mysqlroot' then
begin
global_ini:=myconf.Create();
...
if ParamStr(2)='ssl' then
begin
ccyrus.CYRUS_CERTIFICATE();
halt(0);
end;

if ParamStr(2)='cyrus22' then
begin
GLOBAL_INI:=MyConf.Create();
...
if  ParamStr(1)='--write-versions' then
begin
if not SYS.BuildPids() then
begin halt(0) end;
if debug then
begin writeln('Start --write-versions') end;
GLOBAL_INI:=MyConf.Create();
if debug then
begin writeln('Start CGI_ALL_APPLIS_INSTALLED()') end;
GLOBAL_INI.CGI_ALL_APPLIS_INSTALLED();
if debug then
begin writeln('END CGI_ALL_APPLIS_INSTALLED()') end;
...
if  ParamStr(1)='-geoip-updates' then
begin
...
if ParamStr(1)='--generate-status' then
begin
...
if ParamStr(1)='--generate-status-forced' then
begin
if SYS.isoverloadedTooMuch() then
begin halt(0) end;
