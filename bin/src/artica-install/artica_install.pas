...
uses
  ...
  stunnel4, dkimfilter, postfix_class, mailgraph_daemon, miltergreylist,
  lighttpd, artica_tcp, roundcube, dansguardian, strutils, kav4samba, awstats,
  artica_menus, debian_class, bind9, fdm, collectd, apache_artica,
...
var
 ...
 if ParamCount>0 then begin
     ...
     if ParamStr(1)='--change-ldap-settings' then begin
        zldap:=tOpenldap.Create;
 ...
 // zlogs.Debuglogs('************************************************************');
  ...
  debug:=SYS.COMMANDLINE_PARAMETERS('--debug');
   ...
   if ParamStr(1)='--local-dns' then begin
      GLOBAL_INI:=myconf.Create();
      writeln(GLOBAL_INI.SYSTEM_GET_LOCAL_DNS());
      halt(0);
   end;

    if ParamStr(1)='--dirlists' then begin
      GLOBAL_INI:=myconf.Create();
    ...
    if ParamStr(1)='--phpmyadmin' then begin
      GLOBAL_INI:=myconf.Create();
    ...
    if ParamStr(1)='--php-ini' then begin
      zlighttpd:=Tlighttpd.Create(SYS);
      zlighttpd.LIGHTTPD_ADD_INCLUDE_PATH();
      halt(0);
   end;
   if ParamStr(1)='--squid-version-bin' then begin
      zsquid:=Tsquid.Create;
   ...
   if ParamStr(1)='--monit-check' then begin
      SYS.MONIT_CHECK_ALL();
      halt(0);
   end;


   if ParamStr(1)='--mail-tail' then begin
      zpostfix:=tpostfix.Create(SYS);
      zpostfix.MAILLOG_TAIL(ParamStr(2));
      halt(0);
   end;

   if ParamStr(1)='--gen-cert' then begin
      install.GenerateCertificateFileName(ParamStr(2));
      halt(0);
   end;

    if ParamStr(1)='--all-status' then begin
      GLOBAL_INI:=myconf.Create();
      writeln(GLOBAL_INI.GLOBAL_STATUS());
      halt(0);
   end;

    if ParamStr(1)='--zarafa-certificates' then begin
   ...
   if ParamStr(1)='--milter-grelist-remove' then begin
      milter_greylist:=tmilter_greylist.Create(SYS);
      milter_greylist.REMOVE();
      halt(0);
   end;
   if ParamStr(1)='--postfix-remove' then begin
      zpostfix:=tpostfix.Create(SYS);
      zpostfix.REMOVE();
      halt(0);
   end;
   if ParamStr(1)='--samba-remove' then begin
   ...
   if ParamStr(1)='--kav4Proxy-remove' then begin
      zkav4proxy:=tkav4proxy.Create(SYS);
      zkav4proxy.REMOVE();
      halt(0);
   end;
   if ParamStr(1)='--dansguardian-remove' then begin
      zdansguardian:=tdansguardian.Create(SYS);
      zdansguardian.REMOVE();
      halt(0);
   end;
  if ParamStr(1)='--kav4samba-remove' then begin
      zkav4Samba:=Tkav4Samba.Create;
      zkav4Samba.REMOVE();
      halt(0);
   end;

   if ParamStr(1)='--pureftpd-remove' then begin
        zpureftpd:=Tpureftpd.Create;
        zpureftpd.REMOVE();
        halt(0);
   end;

    if ParamStr(1)='--postfilter-remove' then begin
   ...
   if ParamStr(1)='--dansguardian-template' then begin
      zdansguardian:=Tdansguardian.Create(SYS);
   ...
   if ParamStr(1)='--reconfigure-nic' then begin
      GLOBAL_INI:=myconf.Create;
   ...
   if ParamStr(1)='--reload-cyrus' then begin
      ccyrus.CYRUS_DAEMON_RELOAD();
      halt(0);
   end;

   if ParamStr(1)='--fetchmail-status' then begin
      zfetchmail:=tfetchmail.Create(SYS);
      writeln(zfetchmail.STATUS());
      halt(0);
   end;

   if ParamStr(1)='--retranslator-status' then begin
      retrans:=tkretranslator.Create(SYS);
      writeln(retrans.STATUS());
      halt(0);
   end;

 if ParamStr(1)='--kavm4mls-info' then begin
      zkavm4mls:=tkavm4mls.Create(SYS);
      zkavm4mls.COMPONENTS_INFOS();
      halt(0);
   end;

 if ParamStr(1)='--kavm4mls-pattern' then begin
      zkavm4mls:=tkavm4mls.Create(SYS);
      writeln(zkavm4mls.PATTERN_DATE());
      halt(0);
   end;

   if ParamStr(1)='--kavmilter-pattern' then begin
      zkavmilter:=tkavmilter.Create(SYS);
   ...
   if ParamStr(1)='--reload-spamassassin' then begin
      zSpamass.SPAMASSASSIN_RELOAD();
      halt(0);
   end;

   if ParamStr(1)='--reload-kav4proxy' then begin
       zkav4proxy:=tkav4proxy.Create(SYS);
   ...
   if ParamStr(1)='--reload-assp' then begin
      zassp:=Tassp.Create(SYS);
      zassp.RELOAD();
      halt(0);
   end;

 if ParamStr(1)='--mailboxes-domain' then begin
 ...
 if ParamStr(1)='--mailboxes-list' then begin
      list:=Tstringlist.Create;
 ...
 if ParamStr(1)='--mailbox-delete' then begin
      ccyrus.DELETE_MAILBOXE(ParamStr(2));
      halt(0);
   end;

 if ParamStr(1)='--remote-ressources' then begin
    writeln(SYS.GetRemoteComputerRessources(ParamStr(2),ParamStr(3),ParamStr(4)));
    halt(0);
 end;

 if ParamStr(1)='--mysql-upgrade' then begin
      GLOBAL_INI:=myconf.Create;
      GLOBAL_INI.MYSQL_UPGRADE();
      halt(0);
 end;

 if ParamStr(1)='--vmtools-version' then begin
 ...
 if ParamStr(1)='--kaspersky-status' then begin
      zkas3:=tkas3.Create(SYS);
      zkav4proxy:=tkav4proxy.Create(SYS);
      zkav4Samba:=Tkav4Samba.Create;
      zkavmilter:=tkavmilter.Create(SYS);
     ...
     if ParamStr(1)='-isnetav' then begin
     ...
     if ParamStr(1)='--delete-mailbox' then begin
        ccyrus.DELETE_MAILBOXE(ParamStr(2));
        halt(0);
     end;
     ...
     if ParamStr(1)='--cyrus-checkperms' then begin
        ccyrus.CYRUS_DAEMON_STOP();
        ccyrus.CheckRightsAndConfig();
        ccyrus.CYRUS_DAEMON_START();
        halt(0);
    ...
    if ParamStr(1)='--vmhost' then begin
       writeln(SYS.VMWARE_HOST());
        halt(0);
     end;


     if ParamStr(1)='--check-virus-logs' then begin
        if not SYS.PROCESS_EXIST(SYS.PIDOF('/usr/bin/clamscan')) then begin
             fpsystem(SYS.LOCATE_PHP5_BIN()+' /usr/share/artica-postfix/exec.parse.virus.queue.php &');
        end;
        halt(0);
     end;

     if ParamStr(1)='--scan-cyrus' then begin
     ...
     if ParamStr(1)='-awstats' then install.PARAMETERS_AWSTATS();
     ...
     if ParamStr(1)='--assp-status' then begin
        zassp:=tassp.Create(SYS);
        writeln(zassp.STATUS());
        halt(0);
     end;

     if ParamStr(1)='--pdns-status' then begin
        zpdns:=tpdns.Create(SYS);
        writeln(zpdns.STATUS());
        halt(0);
     end;

     if ParamStr(1)='--gluster-status' then begin
     ...
     if ParamStr(1)='--cluster-replicat-send-ldap' then begin
        ccyrus.CLUSTER_SEND_LDAP_DATABASE();
        halt(0);
     end;

      if ParamStr(1)='--SAVE_SLAPD_CONF' then begin
         zldap:=topenldap.Create;
         zldap.SAVE_SLAPD_CONF();
         halt(0);
      end;

 if ParamStr(1)='--pvcreate-dev' then begin
         zlvm:=tlvm.Create(SYS);
         zlvm.pvcreate_dev(ParamStr(2));
         halt(0);
      end;

 if ParamStr(1)='--vgcreate-dev' then begin
         zlvm:=tlvm.Create(SYS);
 ...
 if ParamStr(1)='--lvm-list' then begin
         zlvm:=tlvm.Create(SYS);
...
if ParamStr(1)='--usb-scan-write' then begin
 ...
 if ParamStr(1)='--ntpd-status' then begin
 ...
 if paramstr(1)='--format-disk-unix' then begin
       SYS.DISK_FORMAT_UNIX(paramstr(2));
       halt(0);
 end;
     
     
     if ParamStr(1)='--urgency-start' then begin
     ...
     if ParamStr(1)='--quarantines-schedule' then begin
        cron:=tcron.Create(SYS);
        cron.quarantine_report_schedules();
        halt(0);
     end;

     if ParamStr(1)='--quarantines-clean-disk' then begin
        GLOBAL_INI:=myconf.Create;
        GLOBAL_INI.CLEAN_QUARANTINES();
        halt(0);
     end;

 if paramStr(1)='--spamassassin-reload' then begin
    zSpamass:=Tspamass.Create(SYS);
    zSpamass.SPAMASSASSIN_RELOAD();
    halt(0);
end;

 if paramStr(1)='--boa-status' then begin
 GLOBAL_INI:=myconf.Create;
 ...
 if paramStr(1)='--sarg-config' then begin
    zsquid:=Tsquid.Create;
    zsquid.SARG_CONFIG();
    writeln('done...');
    halt(0);
 ...
 if paramStr(1)='--reload-rsync' then begin
 ...
 if paramStr(1)='--rsync-status' then begin
    zrsync:=Trsync.Create(SYS);
    writeln(zrsync.STATUS());
    halt(0);
end;
 if paramStr(1)='--free-port' then begin
    writeln(SYS.FREE_PORT());
    halt(0);
end;

 if paramStr(1)='--sarg-scan' then begin
    zsquid:=Tsquid.Create;
  ...
  if paramStr(1)='--apache-groupware-restart' then begin
    xopengoo:=Topengoo.Create(SYS);
    xopengoo.RELOAD();
    halt(0);
end;

  if paramStr(1)='--apache-groupware-php' then begin
    xopengoo:=Topengoo.Create(SYS);
 ...
 if paramStr(1)='--hamachi-nets' then begin
 ...
 if paramStr(1)='--hamachi-status' then begin
     zhamachi:=thamachi.Create(SYS);
     writeln(zhamachi.STATUS());
     halt(0);
 end;

  if paramStr(1)='--apache-ssl-cert' then begin
...
if ParamStr(1)='--status' then begin
   ...
   if SYS.isoverloadedTooMuch() then halt(0);
  ...
  if paramStr(1)='--dstat-gen-mem' then begin
 ...
 if paramStr(1)='--imap-tests' then begin
    writeln(ccyrus.TestingMailBox(paramStr(2),paramStr(3)));
    halt(0);
end;


if paramStr(1)='--xapian-status' then begin
 ...
 if ParamStr(1)='--pid-mem' then begin
 ...
 if paramStr(1)='--obm2-version' then begin
  ...
  if paramStr(1)='--devinfo' then begin
    GLOBAL_INI:=Myconf.Create;
 ...
 if paramStr(1)='--findallpid' then begin
    writeln(SYS.PidAllByProcessPath(ParamStr(2)));
    halt(0);
 end;

 if paramStr(1)='--cups-drivers' then begin
    zcups:=Tcups.Create;
    zcups.CHECK_DRIVERS();
    halt(0);
 end;
 ...
 if paramStr(1)='--cups-config' then begin
    zcups:=Tcups.Create;
 ...
 if paramStr(1)='--cups-delete-all-printers' then begin
    zcups:=Tcups.Create;
 ...
 if paramStr(1)='--cups-windows' then begin
    zcups:=Tcups.Create;
 ...
 if paramStr(1)='--cups-gutenprint' then begin
    zcups:=Tcups.Create;
    zcups.gutenprint_SCAN();
    halt(0);
 end;
 
 if paramStr(1)='--is-ip-forward' then begin
    writeln(SYS.ip_forward_enabled());
    halt(0);
 end;
 
if paramStr(1)='--findpid' then begin
    writeln(SYS.PidByPatternInPath(ParamStr(2)));
    halt(0);
 end;

if paramStr(1)='--samba-status' then begin
    samba3:=Tsamba.Create;
...
if paramStr(1)='--samba-reconfigure' then begin
    samba3:=Tsamba.Create;
    samba3.Reconfigure(true);
    halt(0);
 end;

if paramStr(1)='--php-infos' then begin
...
if ParamStr(1)='--set-loopback' then begin
   zbind9.ApplyLoopBack();
   halt(0);
end;
 
if paramStr(1)='--pidtime' then begin
    writeln('Minutes of this process:',SYS.PROCCESS_TIME_MIN(SYS.PIDOF(ParamStr(2))));
    halt(0);
 end;
 
if paramStr(1)='--mastercf-delete-amavis' then begin
    GLOBAL_INI:=myconf.Create;
...
if paramStr(1)='--amavis-reload' then begin
    GLOBAL_INI:=myconf.Create;
    amavis:=tamavis.Create(GLOBAL_INI.SYS);
...
if paramStr(1)='--kavmilter-reload' then begin
    zkavmilter:=tkavmilter.Create(SYS);
    zkavmilter.RELOAD();
    halt(0);
 end;
 ...
 if ParamStr(1)='--recover-cyrus' then begin
    CCYRUS.REPAIR_CYRUS();
    halt(0);
 end;
 ...
 if paramStr(1)='--cpuinfo' then begin
    writeln(SYS.CPU_MHZ());
    halt(0);
 end;
 
  if paramStr(1)='--lgroup' then begin
    writeln(SYS.IsUserINGroup(ParamStr(2),ParamStr(3)));
    halt(0);
 end;
 
  if paramStr(1)='--whereis-maillog' then begin
    writeln('maillog: ',SYS.MAILLOG_PATH());
    writeln('auth...: ',SYS.LOCATE_AUTH_LOG());
    halt(0);
 end;

   if paramStr(1)='--whereis-syslog' then begin
    writeln(SYS.LOCATE_SYSLOG_PATH());
    halt(0);
 end;
 ...
 if paramStr(1)='--dhcp3-status' then begin
    dhcp3:=tdhcp3.Create(SYS);
    writeln(dhcp3.STATUS());
    halt(0);
 end;
 
 if paramStr(1)='--dns-keygen' then begin
    zbind9:=Tbind9.Create(SYS);
    zbind9.GenerateKey(paramStr(2));
    halt(0);
 end;
 
 if paramStr(1)='--read-queue' then begin
    writeln(zpostfix.POSFTIX_READ_QUEUE(paramStr(2)));
    halt(0);
 end;

 if paramStr(1)='--postfix-sasldb2' then begin
  ...
  if paramStr(1)='--vacation-version' then begin
    writeln(zpostfix.gnarwl_VERSION());
    halt(0);
 end;
 
 if paramStr(1)='--slpapindex' then begin
    zldap:=topenldap.Create;
    zldap.SLAP_INDEX();
    halt(0);
 end;
...
if ParamStr(1)='--enable-postfix-submission' then begin
   if ParamStr(2)='yes' then begin
      writeln('Enable submission in master.cf');
      zpostfix.ENABLE_SUBMISSION(true);
   end else begin
      writeln('Disable submission in master.cf');
...
if ParamStr(1)='--bind9-fw' then begin
   zbind9.ApplyForwarders(ParamStr(2));
   halt(0);
end;

 if paramStr(1)='--mailsync-status' then begin
   ...
   if paramStr(1)='--kretranslator-status' then begin
    retrans:=tkretranslator.Create(SYS);
    writeln(retrans.STATUS());
    halt(0);
 end;
  ...
  if paramStr(1)='--regpid' then begin
    writeln(SYS.AllPidsByPatternInPath(ParamStr(2)));
    halt(0);
 end;
 
  if paramStr(1)='--usbscan' then begin
    GLOBAL_INI:=myconf.Create();
  ...
  if paramStr(1)='--diskscan' then begin
    GLOBAL_INI:=myconf.Create();
  ...
  if paramStr(1)='--atopenmail' then begin
     GLOBAL_INI:=MyConf.Create();
    writeln(GLOBAL_INI.ATMAIL_VERSION());
    halt(0);
 end;
  ...
  if paramStr(1)='--collectd' then begin
     zcollectd:=tcollectd.Create(SYS);
     writeln(zcollectd.PHP_DATA_DIR());
    halt(0);
 end;
 
 if paramStr(1)='--isoqlog' then begin
 ...
 if paramStr(1)='--retranslator' then begin
    ...
    retrans:=tkretranslator.Create(SYS);
    GLOBAL_INI:=myconf.Create();
    if(retrans.RetranslatorEnabled=1) then begin
  ...
  if paramStr(1)='--clamav-status' then begin
     writeln(Zclam.CLAMAV_STATUS());
     writeln(Zclam.FRESHCLAM_STATUS());
     writeln(Zclam.MILTER_STATUS());
    halt(0);
 end;
 
  if paramStr(1)='--mailman-status' then begin
  ...
  if paramStr(1)='--squid-status' then begin
     zdansguardian:=tdansguardian.Create(SYS);
     zkav4proxy:=tkav4proxy.Create(SYS);
  ...
  if paramStr(1)='--obm-status' then begin
     zobm:=tobm.Create(SYS);
     writeln(zobm.STATUS());
    halt(0);
 end;
 
 if paramStr(1)='--get-local-ip' then begin
 ...
 if paramStr(1)='--ldap-reconfigure' then begin
  ...
  if paramStr(1)='--verify-artica-iso' then begin
     deb.linuxlogo();
...
if paramStr(1)='--dansguardian-stats' then begin
        zdansguardian.DANSGUARDIAN_STATS();
        halt(0);
end;

if paramStr(1)='--c-icap-reload' then begin
...
if paramStr(1)='--dansguardian-status' then begin
       writeln(zdansguardian.DANSGUARDIAN_STATUS());
       writeln(zdansguardian.DANSGUARDIAN_TAIL_STATUS());
        halt(0);
end;

if paramStr(1)='--kaspersky' then begin
        install.ARTICA_CD_KASPERSKY();
        halt(0);
end;

if paramStr(1)='--kav4samba-install' then begin
...
if ParamStr(1)='--php-include' then begin
...
if paramStr(1)='--c-icap-configure' then begin
    zdansguardian.C_ICAP_CONFIGURE();
    halt(0);
end;

if paramStr(1)='--spamd-status' then begin
    writeln(zSpamass.MILTER_STATUS());
...
if paramStr(1)='--fdm-status' then begin
...
if paramStr(1)='--fdm-perform' then begin
...
if paramStr(1)='--awstats-generate' then begin
    if SYS.BuildPids() then begin
       zawstats:=tawstats.Create(SYS);
...
if paramStr(1)='--ipstat' then begin
    global_ini:=myconf.Create();
...
if paramStr(1)='--set-hostname' then begin
   global_ini:=myconf.Create();
...
if  paramStr(1)='--setip' then begin
...
if paramStr(1)='--change-certificate' then begin
if SYS.BuildPids() then begin
 ...
 if Paramstr(1)='--openvpn-build-certificate' then begin
    zopenvpn:=topenvpn.Create(SYS);
    zopenvpn.BuildCertificate();
    halt(0);
 end;

  if paramStr(1)='--ldap-ssl' then begin
    zldap.CREATE_CERTIFICATE();
    halt(0);
 end;

if paramStr(1)='--change-postfix-certificate' then begin
   fpsystem('/bin/rm -rf /etc/ssl/certs/postfix/*');
   zpostfix.GENERATE_CERTIFICATE();
   halt(0);
end;

 if ParamStr(1)='-lighttpd-cert' then begin
   GLOBAL_INI:=MyConf.Create();
...
if paramStr(1)='--postfix-status' then begin
   ...
   writeln('GnuPlot version........:',GLOBAL_INI.GNUPLOT_VERSION());
   writeln('GnuPlot version........:',GLOBAL_INI.GNUPLOT_VERSION());
...
if paramStr(1)='--postfix-reload' then begin
   ...
   halt(0);
     ...
     if paramStr(1)='--init-from-repos' then begin
...
if paramstr(1)='--perl-addons-repos' then begin
   install.PERL_LINUX_NET_DEV();
   halt(0);
end;
...
if paramStr(1)='-awstats-reconfigure' then begin
   ...
   halt(0);
     ...
     if paramStr(1)='--init-scripts' then begin
        install.install_init_d();
        halt(0);
     end;
 ...
 if paramStr(1)='--fix-schemas' then begin
     ...
     if paramStr(1)='wget' then begin
        GLOBAL_INI:=MyConf.Create();
        GLOBAL_INI.WGET_DOWNLOAD_FILE(paramStr(2),'/tmp/test.txt');
        halt(0);
     end;
     

 if paramStr(1)='username' then begin
        SYS:=TSystem.Create();
        writeln(SYS.zGetUsername(paramStr(2)));
        halt(0);
     end;
     
     if paramStr(1)='gpid' then begin
        SYS:=TSystem.Create();
        writeln(SYS.IsGroupExists(paramStr(2)));
        halt(0);
     end;
     
     if paramStr(1)='members' then begin
        SYS:=TSystem.Create();
        list:=Tstringlist.Create;
      ...
      if paramStr(1)='--sensors' then begin
         deb.sensors();
         halt(0);
      end;
     
     
     if paramStr(1)='--mysql-reconfigure-db' then begin
...
if paramStr(1)='--interfaces' then begin
...
//##############################################################################
   ...
   if ParamStr(1)='-fdisk' then begin
      SYS:=Tsystem.Create;
...
//##############################################################################
      ...
      if ParamStr(1)='-roundcube' then begin
          if ParamStr(2)='plugins' then begin
...
//######################## K Anti-spam #########################################
if ParamStr(1)='--kas3-status'  then begin
   zkas3:=tkas3.Create(SYS);
   writeln(zkas3.STATUS());
   halt(0);
end;
...
//------------------------------------------------------------------------------
  if ParamStr(1)='--lighttpd-status' then begin
     GLOBAL_INI:=MyConf.Create();
...
if ParamStr(1)='-dkim-status' then begin
...
if ParamStr(1)='--roundcube-permissions' then begin
    GLOBAL_INI:=myconf.Create;
...
if paramStr(1)='--collectd-status' then begin
   GLOBAL_INI:=MyConf.Create();
...
if paramStr(1)='--perl-f' then begin
...
if ParamStr(1)='--squid-reload' then begin
    zsquid.SQUID_RELOAD();
    halt(0);
end;
...
if ParamStr(1)='--kas3-version' then begin
     zkas3:=tkas3.Create(SYS);
...
if ParamStr(1)='--fromcdkav' then begin
...
if ParamStr(1)='-mem' then begin
 ...
 if ParamStr(1)='--generate-status' then begin
     ...
     GLOBAL_INI:=myconf.Create();
     zlogs.WriteToFile(GLOBAL_INI.GLOBAL_STATUS(),GLOBAL_INI.get_ARTICA_PHP_PATH()+'/ressources/logs/global.status.ini');
     zlogs.WriteToFile(GLOBAL_INI.STATUS_PATTERN_DATABASES(),GLOBAL_INI.get_ARTICA_PHP_PATH()+'/ressources/logs/pattern.status.ini');
     fpsystem('/bin/chown www-data:www-data '+GLOBAL_INI.get_ARTICA_PHP_PATH()+'/ressources/logs/*.ini');
     halt(0);
end;
...
if ParamStr(1)='-install-perl-cyrus' then begin
   GLOBAL_INI:=MyConf.Create();
   zMenus:=Tmenus.Create();
...
if ParamStr(1)='-ntpd-install' then begin
   GLOBAL_INI:=MyConf.Create();
   zMenus:=Tmenus.Create();
 ...
 if ParamStr(1)='-listnics' then begin
   GLOBAL_INI:=MyConf.Create();
 ...
 if ParamStr(1)='--syslog' then begin
 ...
 if ParamStr(1)='--stat' then begin
   ...
   if paramStr(1)='--usb-backup' then begin
      if SYS.isoverloaded() then begin
        zlogs.Debuglogs('--usb-backup exiting, system overload');
        exit;
     end;

      if SYS.RotationSeconds(5) then begin
                 GLOBAL_INI:=MyConf.Create();
 ...
 if paramStr(1)='--graphdefang-gen' then begin
     if SYS.RotationSeconds(60) then begin
...
if ParamStr(1)='--startall' then begin
   ...
   if SYS.RotationSeconds(30) then begin
      if SYS.BuildPids() then begin
               zlogs.Debuglogs('#######################################');
 ...
 if ParamStr(1)='--ParseMyqlQueue' then begin
 if SYS.BuildPids() then begin
   if SYS.RotationSeconds(30) then begin
      GLOBAL_INI:=MyConf.Create();
...
if ParamStr(1)='--ln' then begin
 ...
 if ParamStr(1)='--eventsnum' then begin
...
if ParamStr(1)='--roundcube-status' then begin
round:=troundcube.Create(SYS);
...
if ParamStr(1)='--patterns-versions' then begin
   GLOBAL_INI:=MyConf.Create();
