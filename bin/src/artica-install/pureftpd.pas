...
begin
   ...
   ini.Add('[PUREFTPD]');
      ...
      ini.Add('service_cmd=ftp');
      ini.Add('service_disabled='+IntToStr(PureFtpdEnabled));
      ini.Add('remove_cmd=--pureftpd-remove');
     ...
     if STANDALONE_OR_INETD<>'inetd' then begin
      if SYS.MONIT_CONFIG('APP_PUREFTPD',PURE_FTPD_PID_PATH(),'ftp') then begin
         ini.Add('monit=1');
         result:=ini.Text;
         ini.free;
         exit;
 ...
 if STANDALONE_OR_INETD='inetd' then pid:=SYS.INETD_PID() else pid:=PURE_FTPD_PID();
