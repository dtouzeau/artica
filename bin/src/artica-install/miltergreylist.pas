...
begin
       ...
       if MilterGreyListEnabled=1 then begin
          if EnableASSP=1 then begin
             //logs.Debuglogs('tmilter_greylist.Create() :: ASSP is enabled, disable milter-greylist');
             MilterGreyListEnabled:=0;
          end;
...
begin
   ...
   ini.Add('[MILTER_GREYLIST]');
      ...
      ini.Add('remove_cmd=--milter-grelist-remove');
     ...
     if MilterGreyListEnabled=0 then begin
      ...
      if SYS.MONIT_CONFIG('APP_MILTERGREYLIST',MILTER_GREYLIST_PID_PATH(),'mgreylist') then begin
         ini.Add('monit=1');
         result:=ini.Text;
         ini.free;
         exit;
...
daemon_bin:=MILTER_GREYLIST_BIN_PATH();
 ...
 for i:=0 to l.Count-1 do begin
   ...
   if RegExpr.Exec(l.Strings[i]) then begin
      ...
      logs.DebugLogs('Starting......: milter-greylist syntax error line '+RegExpr.Match[1]+' remove it');
