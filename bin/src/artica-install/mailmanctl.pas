...
var
   ...
   RegExpr        :TRegExpr;
   servername:string;
   DEFAULT_EMAIL_HOST,MAILMAN_DEFAULT_URL_PATTERN,MAILMAN_PUBLIC_ARCHIVE_URL,MAILMAN_DEFAULT_URL_HOST:string;
begin

  DEFAULT_EMAIL_HOST:=trim(SYS.GET_INFO('MAILMAN_DEFAULT_EMAIL_HOST'));
  MAILMAN_DEFAULT_URL_PATTERN:=trim(SYS.GET_INFO('MAILMAN_DEFAULT_URL_PATTERN'));
  MAILMAN_PUBLIC_ARCHIVE_URL:=trim(SYS.GET_INFO('MAILMAN_PUBLIC_ARCHIVE_URL'));
  MAILMAN_DEFAULT_URL_HOST:=trim(SYS.GET_INFO('MAILMAN_DEFAULT_URL_HOST'));
...
if SYS.MONIT_CONFIG('APP_MAILMAN',PID_PATH(),'mailman') then begin
   ini.Add('monit=1');
   result:=ini.Text;
   ini.free;
   exit;
