...
if hasProxyTransparent=1 then begin
    ...
    if not IS_IPTABLES_SQUID_EXISTS(local_port) then begin
       logs.Syslogs('Starting......: Insert routing table in iptables configuration... dest:'+local_port);
       fpsystem(SYS.LOCATE_IPTABLES() + ' -A PREROUTING -t nat -p tcp --dport 80 -j REDIRECT --to-port '+local_port);
    end else begin
       logs.Debuglogs('Starting......: Insert routing table in iptables already done...');
    end;
end else begin
...
if not SYS.PROCESS_EXIST(pid) then begin
     writeln('Stopping Squid...............: Stopped');
     TAIL_STOP();
  end;
end;
//##############################################################################
...
begin
 ...
 pidpath:=SQUID_GET_CONFIG('pid_filename');
 if SYS.MONIT_CONFIG('APP_SQUID',pidpath,'squid') then begin
    ini.Add('monit=1');
    result:=ini.Text;
    ini.free;
    exit;
   ...
   ini.Add('service_disabled='+IntToStr(SQUIDEnable));
   ini.Add('pid_path='+SQUID_GET_CONFIG('pid_filename'));
