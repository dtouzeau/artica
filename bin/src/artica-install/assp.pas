...
begin
       ...
       if not TryStrToInt(SYS.GET_INFO('NotASSPRemovePass'),NotASSPRemovePass) then NotASSPRemovePass:=0;
       if not TryStrToInt(SYS.GET_INFO('EnableASSPBackup'),EnableASSPBackup) then EnableASSPBackup:=0;
...
var
    ...
    debug:boolean;
begin
if not FileExists('/usr/share/assp/assp.pl') then exit;
...
begin
if not TryStrToInt(SYS.GET_INFO('ASSPDoLDAP'),ASSPDoLDAP) then ASSPDoLDAP:=0;
if not TryStrToInt(SYS.GET_INFO('ASSPEnableDelaying'),ASSPEnableDelaying) then ASSPEnableDelaying:=0;
if not TryStrToInt(SYS.GET_INFO('ASSPMessageScoringLowerLimit'),MessageScoringLowerLimit) then MessageScoringLowerLimit:=50;
if not TryStrToInt(SYS.GET_INFO('ASSPMessageScoringUpperLimit'),MessageScoringUpperLimit) then MessageScoringUpperLimit:=60;
...
if EnableASSPBackup=0 then begin
...
if FileExists('/etc/artica-postfix/settings/Daemons/ASSPDelayingConfig') then begin
...
for i:=0 to z.Count-1 do begin
    ...
    if not RegExpr.Exec(ipstr) then continue;
...
if FileExists('/etc/artica-postfix/mynetworks') then begin
     z.LoadFromFile('/etc/artica-postfix/mynetworks');
     for i:=0 to z.Count-1 do begin
       if(length(z.Strings[i]))=0 then continue;
       if z.Strings[i]='0.0.0.0' then continue;
       if not RegExpr.Exec(z.Strings[i]) then begin
...
if Copy(noSpoofingCheckIP,length(noSpoofingCheckIP),1)='|' then begin
     noSpoofingCheckIP:=Copy(noSpoofingCheckIP,1,length(noSpoofingCheckIP)-1);
  end;

logs.DebugLogs('tassp.InternalsIPs() mynets='+mynets);
if Copy(mynets,length(mynets),1)='|' then begin
...
begin
  ...
  if not SYS.PROCESS_EXIST(pid) then begin
     ...
     for i:=0 to pids.Count-1 do begin
         if length(trim(pids.Strings[i]))=0 then continue;
         if SYS.PROCESS_EXIST(pids.Strings[i]) then begin
...
if not FileExists('/usr/share/assp/assp.pl') then exit;
 ...
 if SYS.MONIT_CONFIG('APP_ASSP','/usr/share/assp/pid','assp') then begin
    ini.Add('monit=1');
    result:=ini.Text;
    ini.free;
    exit;
...
var
    ...
    found:boolean;
begin
    if NotASSPRemovePass=1 then exit;
    if not FileExists('/usr/share/assp/assp.cfg') then exit;
    FileDatas:=TStringList.Create;
