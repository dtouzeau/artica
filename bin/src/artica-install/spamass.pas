...
if not FileExists(SPAMASSASSIN_LOCAL_CF()) then begin
   logs.Syslogs('Starting......: Unable to stat spamassassin local.cf');
   exit;
end;
...
if not IF_PATTERN_FOUND('^rewrite_header Subject') then begin
      logs.Syslogs('Starting......: Set default rewrite_header parameter');
      l.Add('rewrite_header Subject ***** SPAM *****');
end;

if not IF_PATTERN_FOUND('^required_score') then begin
   l.Add('required_score 5.0');
   logs.Syslogs('Starting......: Set default required_score parameter');
end;

if not IF_PATTERN_FOUND('^report_safe') then begin
   l.Add('report_safe 0');
   logs.Syslogs('Starting......: Set default report_safe parameter');
end;
...
if not IF_PATTERN_FOUND('^bayes_ignore_header') then begin
   ...
   logs.Syslogs('Starting......: Set default bayes_ignore_header');
end;
...
logs.Syslogs('Starting......: Unable to save configuration file '+SPAMASSASSIN_LOCAL_CF());
end;
l.free;
end;
//#########################################################################################
...
if not FileExists(filepath) then begin
      logs.Syslogs('Starting......: Unable to stat file '+filepath);
      exit;
end;
