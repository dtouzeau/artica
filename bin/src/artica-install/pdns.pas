...
uses
    Classes, SysUtils,variants,strutils,IniFiles, Process,logs,unix,RegExpr,zsystem,openldap,tcpip,bind9;
...
public
    ...
    function    STATUS_RECURSOR:string;
END;
...
begin
   ...
   if not fileExists(BIN_PATH()) then begin
      ...
      ini.Add('service_name=APP_PDNS');
      ini.Add('service_cmd=pdns');
      ini.Add('service_disabled=1');

      if SYS.MONIT_CONFIG('APP_PDNS','/var/run/pdns/pdns.pid','pdns') then begin
       ini.Add('monit=1');
       result:=ini.Text;
       exit;
      end;
      ...
      ini.Add('pid_path=/var/run/pdns/pdns.pid');
...
//#############################################################################
function tpdns.STATUS_RECURSOR:string;
var
ini:TstringList;
pid:string;
begin
  ...
  ini.Add('[PDNS_RECURSOR]');
  ini.Add('master_version='+VERSION());
  ini.Add('service_cmd=pdns');
  ini.Add('service_disabled=1');
  ini.Add('service_name=APP_PDNS_RECURSOR');

  if SYS.MONIT_CONFIG('APP_PDNS_RECURSOR','/var/run/pdns/pdns_recursor.pid','pdns') then begin
     ini.Add('monit=1');
     result:=ini.Text;
     exit;
  end;
  ...
  ini.Add('master_memory=' + IntToStr(SYS.PROCESS_MEMORY(pid)));
  ini.Add('status='+SYS.PROCESS_STATUS(pid));
  ini.Add('pid_path=/var/run/pdns/pdns_recursor.pid');
  result:=ini.Text;
   ini.free;
