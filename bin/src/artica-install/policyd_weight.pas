...
begin
     ...
     if length(trim(result))=0 then result:=SYS.PIDOF(BIN_PATH());
end;
//##############################################################################
...
if not FileExists(BIN_PATH()) then exit;
      ...
      ini.Add('service_cmd=policydw');
      ini.Add('service_disabled='+IntToStr(EnablePolicydWeight));
...
pid_path:=GET_VALUE('PIDFILE');
if SYS.MONIT_CONFIG('APP_POLICYD_WEIGHT',pid_path,'policydw') then begin
       ini.Add('monit=1');
      result:=ini.Text;
      ini.free;
      exit;
