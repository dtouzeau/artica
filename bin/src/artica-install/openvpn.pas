  ...
  if RegExpr.Exec(server_name) then begin
   ...
   forceDirectories('/var/run/openvpn');
    ...
    if DEV_TYPE<>'tap0' then begin
       ...
       logs.Debuglogs('Starting......: OpenVPN Bridged is not set, change to tun');
       cmd:=BIN_PATH()+' '+BuildCommandLIneTUN();
    end else begin
        logs.OutputCmd(BIN_PATH()+' --mktun --dev tap0');
        CREATE_BRIDGE();
        cmd:=BIN_PATH()+' '+BuildCommandLIneTAP();
    end;
...
function topenvpn.BuildCommandLIneTAP():string;
begin
fpsystem(SYS.LOCATE_PHP5_BIN()+' /usr/share/artica-postfix/exec.openvpn.php --server-conf');
result:=logs.ReadFromFile('/etc/openvpn/cmdline.conf');
end;
...
if not FileExists(BIN_PATH()) then exit;
   ...
   ini.Add('[OPENVPN_SERVER]');
   ini.Add('master_version=' + VERSION());
   ini.Add('service_name=APP_OPENVPN');
   ini.Add('service_cmd=openvpn');
   ini.Add('service_disabled='+ IntToStr(EnableOPenVPNServerMode));
   ...
   if SYS.MONIT_CONFIG('APP_OPENVPN','/var/run/openvpn/openvpn-server.pid','openvpn') then begin
      ini.Add('monit=1');
      result:=ini.Text;
      ini.free;
      exit;
