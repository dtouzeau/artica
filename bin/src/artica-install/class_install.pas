...
uses
  ...
  jcheckmail,dhcp_server,obm2,dstat,rsync,smartd,tcpip,policyd_weight,apache_artica,autofs,framework,assp,pdns,gluster,nfsserver,zabbix,hamachi,postfilter, vmwaretools,zarafa_server,monit,
...
begin
       ...
       if currentpath[length(currentpath)]='/' then currentpath:=copy(currentpath,0,length(currentpath)-1);
       currentpath:=ExtractFilePath(currentpath);
       if currentpath[length(currentpath)]='/' then currentpath:=copy(currentpath,0,length(currentpath)-1);
       GLOBALINI.set_INSTALL_PATH(currentpath);
    ...
    monit:=tmonit.Create(SYS);
          ...
          if ParamStr(2)='p3scan' then begin
            ...
            if length(ParamStr(2))>0 then begin
               ...
               writeln('|zabbix|hamachi|kav4proxy|postfilter|vmtools|zarafa|zarafa-web|monit');
               exit();
            end;
    ...
    if not NetWorkAvailable then begin
       writeln('Warning, Network is unreachable, some services could not be start...');
             ...
             if ParamStr(2)='ldap' then begin
              ...
              if ParamStr(2)='postfix-single' then begin
                  if NetWorkAvailable then zpostfix.POSTFIX_START();
                  exit();
              end;
        ...
        if length(ParamStr(2))>0 then begin
               ...
               writeln('|syslogger|zabbix|kav4proxy|postfilter|vmtools|zarafa|zarafa-web|monit');
               exit();
            end;
...
begin
 ...
 if geoip_new_date=geoip_date then begin
   ...
   if geoip_new_date<geoip_date then begin
     ...
     LOGS.syslogs('Downloading updates...Finish');
     fpsystem('cd '+tempdir+' && /bin/gunzip -f -q GeoIP.dat.gz GeoLiteCity.dat.gz GeoIPASNum.dat.gz');
...
begin
  ...
  logs.Debuglogs('Change Zarafa certificate');
...
begin
 ...
 if not fileExists('/usr/share/artica-postfix') then begin
  ...
  myFile.Add('#!/bin/sh');
  myFile.Add('#Begin /etc/init.d/artica-postfix');
