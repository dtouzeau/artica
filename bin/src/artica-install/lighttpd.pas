...
uses
    Classes, SysUtils,variants,strutils,IniFiles, Process,md5,logs,unix,RegExpr in 'RegExpr.pas',zsystem,awstats,mailmanctl,tcpip,openldap,Baseunix,mysql_daemon,zarafa_server;
...
logs.Debuglogs('###################### LIGHTTPD #####################');
   ...
   fpsystem('/bin/chmod 755 /usr/share/artica-postfix/user-backup/ressources/conf');
...
var
   ini:TstringList;
   ApacheArticaEnabled:integer;
begin
  if not FileExists(LIGHTTPD_BIN_PATH()) then exit;
  ini:=TstringList.Create;
  ini.Add('[LIGHTTPD]');
  ini.Add('service_disabled='+IntToStr(EnableLighttpd));
  ini.Add('service_name=APP_LIGHTTPD');
  ini.Add('service_cmd=apache');
  ini.Add('master_version=' + LIGHTTPD_VERSION());
  if not TryStrToInt(SYS.GET_INFO('ApacheArticaEnabled'),ApacheArticaEnabled) then ApacheArticaEnabled:=0;
  if ApacheArticaEnabled=1 then EnableLighttpd:=0;
     ...
     if EnableLighttpd=0 then begin
      ...
      if SYS.MONIT_CONFIG('APP_LIGHTTPD',LIGHTTPD_PID_PATH(),'apache') then begin
         ini.Add('monit=1');
         result:=ini.Text;
         ini.free;
         exit;
...
l.Add('include_path = ".:/usr/share/php:/usr/share/obm:/usr/share/php5:/usr/share/obm2:/usr/local/share/php"');
l.Add('doc_root =');
l.Add('user_dir =');
...
for i:=0 to z.Count-1 do begin
     ...
     if FileExists(sofile)  then begin
        ...
        logs.WriteToFile(l.Text,confdir+'/'+soname);
        if DirectoryExists('/etc/php5/cgi/conf.d') then logs.WriteToFile(l.Text,'/etc/php5/cgi/conf.d/'+soname);
        if DirectoryExists('/etc/php5/cli/conf.d') then logs.WriteToFile(l.Text,'/etc/php5/cli/conf.d/'+soname);
