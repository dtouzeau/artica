...
if EnableScanSecurity=1 then begin
   ...
   if FileExists('/etc/artica-postfix/sanesecurity_time') then begin
      if SYS.FILE_TIME_BETWEEN_MIN('/etc/artica-postfix/sanesecurity_time')>420 then begin
         logs.OutputCmd('/usr/share/artica-postfix/bin/scamp.sh');
...
//Kaspersky update
  ...
  if ParamStr(1)='--package' then begin
      if FileExists('/opt/artica/sources/auto-update/'+ParamStr(2)) then begin
         ...
         logs.mysql_logs('3','1','success extract and update'+ParamStr(2));
         halt(0);
      end;
  end;
  ...
  if GLOBAL_INI.COMMANDLINE_PARAMETERS('--force') then begin
     ...
     logs.mysql_logs('3','2','--force was executed by shell...');
  end;
  ...
  if Beetwen>=MaxMin then begin
    ...
    if lastest>current then begin
       ...
       if not FileExists('/opt/artica/sources/auto-update/'+lastestfile) then begin
         ...
         if autoinstall then begin
          ...
          logs.mysql_logs('3','1','success store update package '+lastestfile + ' waiting installation order');
    ...
    if GLOBAL_INI.COMMANDLINE_PARAMETERS('--force') then logs.mysql_logs('3','2','Nothing to do ...');
    end;
...
if LastestBuild=MyCurrentVersion then begin
    logs.Debuglogs('NightlyBuild():: up-to-date... aborting ('+Lastest+')');
    exit;
end;
...
if LastestBuild<MyCurrentVersion then begin
  ...
  logs.Debuglogs('Downloading new version ' + Lastest);
  uri:='http://www.artica.fr/nightbuilds/artica-'+Lastest+'.tgz';
  forceDirectories('/tmp/'+Lastest);

  ArticaFileTemp:='/tmp/'+Lastest+'/'+Lastest+'.tgz';
  logs.Debuglogs('downloading....');
  GLOBAL_INI.WGET_DOWNLOAD_FILE(uri,ArticaFileTemp);
  logs.Debuglogs('Stopping artica...');
  fpsystem('/etc/init.d/artica-postfix stop');
  logs.Debuglogs('Stopping artica twice ...');
  fpsystem('/etc/init.d/artica-postfix stop');
  logs.Debuglogs('decompressing '+ArticaFileTemp );


  if not FileExists(ArticaFileTemp) then begin
     logs.Debuglogs('Corrupted package '+ArticaFileTemp);
     exit;
  end;

  writeln('Extracting package, please wait...');
 ...
 if LastestBuild=MyCurrentVersion then begin
    logs.Debuglogs('NightlyBuild():: success');
...
begin
  ...
  logs.mysql_logs('3','2','uncompress file '+lastestfile);
  current:=GetCurrentVersion();
  ...
  current:=GetCurrentVersion();
          if lastest=current then begin
              logs.mysql_logs('3','2','Restarting artica...');
              ...
              logs.mysql_logs('3','1','success updated to version '+GLOBAL_INI.ARTICA_VERSION());
              if D then writeln('Success ! -> remove file');
              GLOBAL_INI.DeleteFile('/opt/artica/sources/auto-update/'+lastestfile);
              logs.mysql_logs('3','2','success update artica...');
              ...
              logs.mysql_logs('3','0','Failed extract '+lastestfile);
...
if FileExists('/usr/share/artica-postfix/bin/install/amavis/sa-channels.conf') then begin
   logs.OutputCmd(spamassassin.SA_UPDATE_PATH()+' --channelfile /usr/share/artica-postfix/bin/install/amavis/sa-channels.conf --gpgkey 856AA88A');
end else begin
...
if not FileExists(squidguard.BIN_PATH()) then begin
    logs.Debuglogs('update_squidguard is not installed');
    exit;
end;
...
fpsystem(squidguard.BIN_PATH()+' -P -C all &');


if squidGuardEnabled=1 then logs.NOTIFICATION('[ARTICA]:('+sys.HOSTNAME_g()+') success update  squidGuard ' + IntToStr(e.Count)+ ' URLs databases' ,e.Text,'update');
    RegExpr.free;
    d.free;
    l.free;
    e.free;
...
squidguard:=tsquidguard.Create(SYS);
if FileExists(squidguard.BIN_PATH()) then begin
    logs.Debuglogs('update_webfilterplus:: SquidGuard is installed');
