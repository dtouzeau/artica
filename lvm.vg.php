...
function js(){
	...
	$page=CurrentPageName();
	$tpl=new templates();
	$title=$tpl->_ENGINE_parse_body('{APP_LVM}','system.internal.disks.php');
	...
	dev_mem='';
		...
		function {$suffix}LoadPage(){
			YahooWin5(650,'$page?popup=yes&dev=$dev','$title');
		}
...
function popup(){
	...
	$dev=$_GET["dev"];
	$sock=new sockets();
	$datas=trim($sock->getfile("vgdisplay:$dev"));
	$tbl=explode("\n",$datas);
	$lvm_ou=new lvm_org();
	...
	$dfmoinshdev=$sock->getfile("dfmoinshdev:$devmapper");
	$df_ar=explode("\n",$dfmoinshdev);
	...
	$t="<table style='width:100%' class=table_form>";
	...
	if($array["LV UUID"]<>null){
		$devmapper=$array["dev mapper"];
		$uuid=$array["LV UUID"];
	...
	<table style='width:100%'>
		<tr>
		<td valign='top' width=1%><br>$format<br><br>$connect<br></td>
