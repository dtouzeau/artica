...
if (isset ( $_GET ["ChangeUserPasswordSave"] )) {
	USER_CHANGE_PASSWORD_SAVE();
	exit ();
}

if (isset ( $_GET ["UserChangeEmailAddr"] )) {
	USER_CHANGE_EMAIL ();
	exit ();
}
if (isset ( $_GET ["UserChangeEmailAddrSave"] )) {
	USER_CHANGE_EMAIL_SAVE ();
	exit ();
}

if (isset ( $_GET ["zarafa-mailbox-edit"] )) {
	ZARAFA_MAILBOX_EDIT_JS ();
	exit ();
}
if (isset ( $_GET ["zarafaQuotaWarn"] )) {
	ZARAFA_MAILBOX_SAVE ();
	exit ();
}

if (isset ( $_GET ["loginShell"] )) {
	USER_SYSTEM_INFOS_SAVE ();
	exit ();
}

if (isset ( $_GET ["FinalDateToLive"] )) {
	USER_SYSTEM_INFOS_SAVE ();
	exit ();
}
if (isset ( $_GET ["MailAlternateAddress"] )) {
	USER_ALTERNATE_EMAIL_SAVE ();
	exit ();
}
if (isset ( $_GET ["UserEndOfLIfe"] )) {
	USER_ENDOFLIFE ();
	exit ();
}
...
if (isset ( $_GET ["RecipientToAdd_delete"] )) {
	USER_BBC_MAP_DEL ();
	exit ();
}
if (isset ( $_GET ["USER_BBC_MAP_LIST"] )) {
	echo USER_BBC_MAP_LIST ();
	exit ();
}

if (isset ( $_GET ["sync_next_user"] )) {
	TOOL_SYNC_STEP2 ();
	exit ();
}
if (isset ( $_GET ["imapsync_events"] )) {
	TOOL_SYNC_EVENTS ();
	exit ();
}
if (isset ( $_GET ["sync_find_user"] )) {
	TOOL_SYNC_FIND_MAILBOX ();
	exit ();
}
if (isset ( $_GET ["sendparams"] )) {
	USER_SENDER_PARAM ( $_GET ["userid"] );
	exit ();
}
if (isset ( $_GET ["ComputerAddAlias"] )) {
	COMPUTER_ADD_ALIAS ();
	exit ();
}
if (isset ( $_GET ["new_userid"] )) {
	USER_ADD ();
	exit ();
}

if (isset ( $_GET ["DotClearUserEnabled"] )) {
	SaveLdapUser ();
	exit ();
}
if (isset ( $_GET ["SaveLdapUser"] )) {
	SaveLdapUser ();
	exit ();
}
if (isset ( $_GET ["SaveUserInfos"] )) {
	SaveUserInfos ();
	exit ();
}
...
if (isset ( $_GET ["create-mailbox-wizard"] )) {
	USER_MAILBOX_WIZARD_JS ();
	exit ();
}
if (isset ( $_GET ["create-mailbox-step1"] )) {
	USER_MAILBOX_WIZARD_STEP1 ();
	exit ();
}
if (isset ( $_GET ["UserMailBoxEdit"] )) {
	UserMailBoxEdit ();
	exit ();
}
if (isset ( $_GET ["debug-mailbox-js"] )) {
	UserMailBoxDebugJs ();
	exit ();
}
if (isset ( $_GET ["debug-mailbox-user"] )) {
	UserMailBoxDebugEvents ();
	exit ();
}
...
if (isset ( $_GET ["DeleteThisUser"] )) {
	USER_DELETE ();
	exit ();
}
//FTP
if (isset ( $_GET ["UserFTPEdit"] )) {UserFTPEdit();exit ();}
if (isset ( $_GET ["SambaUid"] )) {
	USER_SAMBA_EDIT ();
	exit ();
}
if (isset ( $_GET ["RebuildSambaFields"] )) {
	USER_SAMBA_REBUILD_NULL ();
	exit ();
}
if (isset ( $_GET ["smb-section"] )) {
	echo USER_SAMBA ( $_GET ["userid"] );
	exit ();
}
if (isset ( $_GET ["recipient_translations"] )) {
	echo USER_RECIPT_PARAM ( $_GET ["userid"] );
	exit ();
}

if (isset ( $_GET ["AJAX_COMPUTER_MATERIAL_OS_SAVE"] )) {
	AJAX_COMPUTER_MATERIAL_OS_SAVE ();
	exit ();
}
if (isset ( $_GET ["SaveComputerInfo"] )) {
	COMPUTER_SAVE_INFOS ();
	exit ();
}
if (isset ( $_GET ["NmapScanComputer"] )) {
	COMPUTER_NMAP ();
	exit ();
}
...
if (isset ( $_GET ["DeletComputerAliases"] )) {
	COMPUTER_DELETE_ALIAS ();
	exit ();
}
if (isset ( $_GET ["DeleteSenderCanonical"] )) {
	USER_DELETE_CANONICAL ();
	exit ();
}
...
if (isset ( $_GET ["TOOLS_REPAIR"] )) {
	TOOLS_REPAIR ();
	exit ();
}
if (isset ( $_GET ["TOOLS_SYNC"] )) {
	TOOLS_SYNC ();
	exit ();
}
if (isset ( $_GET ["TOOLS_IMPORT"] )) {
	TOOLS_IMPORT ();
	exit ();
}
...
//SAMBA
if (isset ( $_GET ["enable-shared"] )) {
	USER_SAMBA_ENABLE_JS ();
	exit ();
}
		...
		if ($user->NotASambaUser) {
			...
			<tr>
				...
				<td style='font-size:13px;color:#D45D17'>
					...
					<i style='font-size:10px;color:#D45D17'>{this_not_a_samba_user_explain}</i>
					
					<div style='text-align:right'>
						<input type='button' style='margin:0;font-size:10px' value='{activate}&nbsp;&raquo;&raquo;' OnClick=\"javascript:Loadjs('$page?enable-shared=yes&userid=$userid')\">
					</div>
					</td>
			</tr>";
		}
	...
	while ( list ( $num, $ligne ) = each ( $tb ) ) {
		if (trim ( $ligne ) != null) {
			if (preg_match ( '#(.+?):(.+)#', $ligne, $re )) {
				...
				$table = $table . "<div style='width:240px;border-bottom:1px dotted #CCCCCC''>$ligne</div>\n";
			}
...
function USER_CHANGE_PASSWORD_SAVE() {
	...
	writelogs("{$_GET["uid"]} password=".strlen($_GET["ChangeUserPasswordSave"])." length",__FUNCTION__,__FILE__,__LINE__);
...
function USER_CHANGE_PASSWORD() {
	...
	<tr>
		...
		<td>" . Field_password ("UserPassword", $ct->password, "font-size:13px;padding:5px;width:110px" ) . "</td>
	</tr>
	<tr>
		<td colspan=2 align='right'>
...
function USER_MESSAGING($userid) {
	...
	if (! $usermenus->AllowChangeAntiSpamSettings) {
		$AmavisSettings = null;
	}
	if (! $usermenus->AMAVIS_INSTALLED) {
		$AmavisSettings = null;
	}
...
function USER_ACCOUNT($userid) {
 ...
 $emule=Paragraphe( "64-emule.png", '{MLDONKEY_USER}',
	...
	$password = Paragraphe ( "64-ssl-key.png", '{change_password}', '{change_password_text}', "javascript:ChangeUserPassword('$userid');", null, 210, 'font-size:12px;font-weight:bold' );
	...
	if(!$priv->MLDONKEY_INSTALLED){$emule==null;}else{
		$sock=new sockets();
...
function USER_MAILBOX_NONEXISTENT($uid) {
	...
	" . Paragraphe ( "inbox-error-64.png", "{no_mailbox}", "{user_no_mailbox}", "javascript:Loadjs('$page?create-mailbox-wizard=yes&uid=$uid')", "{create_mailbox}" ) . "
	</center>";
...
function USER_CHANGE_UID() {
	$uid = $_GET ["userid"];
	$html = "
	<p style='font-size:13px'>{change_uid_explain}</p>
	<div id='chuiseriddiv'>
	<table style='width:100%' class=table_form>
	<tr>
...
function USER_PRIVILEGES() {
	...
	if ($users->AllowEditOuSecurity) {
		$AllowEditOuSecurity = "status_ok.gif";
	}
	if ($users->AsOrgPostfixAdministrator) {
		$AsOrgPostfixAdministrator = "status_ok.gif";
	}
	if ($users->AsQuarantineAdministrator) {
		$AsQuarantineAdministrator = "status_ok.gif";
	}
	if ($users->AsMailManAdministrator) {
		$AsMailManAdministrator = "status_ok.gif";
	}
	if ($users->AsOrgStorageAdministrator) {
		$AsOrgStorageAdministrator = "status_ok.gif";
	}
	if ($users->AsMessagingOrg) {
		$AsMessagingOrg = "status_ok.gif";
	}
	if ($users->AllowAddUsers) {
		$AllowAddUsers = "status_ok.gif";
	}
	if ($users->AsDansGuardianGroupRule) {
		$AsDansGuardianGroupRule = "status_ok.gif";
	}
	if ($users->AsOrgAdmin) {
		$AsOrgAdmin = "status_ok.gif";
	}
...
<table style='width:100%' class=table_form>	
	...
	if ($users->AllowChangeAntiSpamSettings) {
		$AllowChangeAntiSpamSettings = "status_ok.gif";
	}
	if ($users->AllowChangeUserPassword) {
		$AllowChangeUserPassword = "status_ok.gif";
	}
	if ($users->AllowFetchMails) {
		$AllowFetchMails = "status_ok.gif";
	}
	if ($users->AllowChangeUserKas) {
		$AllowChangeUserKas = "status_ok.gif";
	}
	if ($users->AllowChangeMailBoxRules) {
		$AllowChangeMailBoxRules = "status_ok.gif";
	}
	if ($users->AllowSenderCanonical) {
		$AllowSenderCanonical = "status_ok.gif";
	}
	if ($users->AllowOpenVPN) {
		$AllowOpenVPN = "status_ok.gif";
	}
	if ($users->AllowDansGuardianBanned) {
		$AllowDansGuardianBanned = "status_ok.gif";
	}
	if ($users->AllowXapianDownload) {
		$AllowXapianDownload = "status_ok.gif";
	}
	if ($users->AllowEditAsWbl) {
		$AllowEditAsWbl = "status_ok.gif";
	}
