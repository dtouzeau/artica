...
function quarantine_show(){
	...
	$tpl=new templates();
	$q=new mysql();
	$sq="SELECT message_path FROM orgmails WHERE MessageID='$message_id'";
	$ligne=@mysql_fetch_array($q->QUERY_SQL($sq,"artica_backup"));
	$path=$ligne["message_path"];
	
	$sql="SELECT MessageBody,file_path FROM quarantine WHERE MessageID='$message_id'";
	...
	". button("{release}","QReleaseMail('$message_id')")."
	</div>
...
while($ligne=mysql_fetch_array($results,MYSQL_ASSOC)){
		...
		"<tr " . CellRollOver("AdminQuarantine('{$ligne["MessageID"]}')").">
