...
function LoadMainLabel(){
	YahooWin6('300','$page?change-label-popup={$_GET["uuid"]}','$title {$_GET["uuid"]}');
	}	
...
function uuid_js(){
	...
	$html="
		function {$prefix}LoadPage(){
			YahooLogWatcher(700,'$page?uuid-popup=$uuid','$title');
...
if($users->autofs_installed){
	...
	if($usb->mounted<>null){
			...
			$browse="<tr><td colspan=2 align=right><input type='button' OnClick=\"javascript:$js_brows\" value='{browse}...'></td></tr>";
	}
			
	
	$html="<H1>$usb->LABEL</H1>
	...
	<tr>
		...
		<td valign='top'>
			...
			<tr>
				...
				<td><strong>$usb->LABEL</strong></td>
			</tr>
			<tr>
				<td class=legend>{path}:</td>
				<td><strong>$usb->path ($usb->ID_FS_TYPE)</strong></td>
			</tr>			
			<tr>
				<td class=legend>{size}:</td>
				<td><strong>$usb->size ($usb->pourc%)</strong></td>
			</tr>	
			<tr>
				<td class=legend>{mounted}:</td>
				<td><strong>$usb->mounted</strong></td>
			</tr>					
			<tr>
				<td class=legend>{manufacturer}:</td>
				<td><strong>$usb->vendor</strong></td>
			</tr>	
			<tr>
				<td class=legend>{model}:</td>
				<td><strong>$usb->model</strong></td>
			</tr>		
			<tr>
				<td class=legend>{product}:</td>
				<td><strong>$usb->product</strong></td>
			</tr>	
			$browse					
			</table>
		</td>
