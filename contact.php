...
if(isset($_GET["xEditSave"])){CONTACT_SAVE();exit;}
...
function js(){
	...
	$html="
		...
		function LoadContact(){
			YahooWin5(750,'$page?contact=yes&$uri','$title');
			
		
		}
		
		function CompleteName(){
			YahooWin6(350,'$page?completeName=yes','$title');
		...
		function ContactTabs(num){
			...
			if(document.getElementById('uidUser')){
				...
				LoadAjax('contact_section','$page?section='+num+'&uidUser='+uidUser);
				LoadAjax('tabs','$page?section='+num+'&uidUser='+uidUser+'&showtab=yes');
		...
		function EditContact(Form_name){
			...
			if(document.getElementById('uidUser')){
				var uidUser=document.getElementById('uidUser').value;
				ParseForm(Form_name,'$page',true,false,false,'contact_section','$page?section='+mem_section+'&uidUser='+uidUser,x_EditContact);
			}			
...
function CONTACT_COMPLETE_NAME(){
	...
	<table style='width:100%' class=table_form>
		...
		<tr>
			<td align='right' colspan=2><input type='button' OnClick=\"javascript:CompleteNameEdit();\" value='{edit}&nbsp;&raquo;'>
		</tr>	
	</table>	
	</div>
...
if(($_GET["section"]==null)){$_GET["section"]="contact";}
	
	
	$arr["contact"]="{contact}";
	$arr["personal-informations"]="{personal_informations}";
	$arr["postal-address"]="{postal_address}";
	
	while (list ($num, $ligne) = each ($arr) ){
		if($_GET["section"]==$num){$class="id=tab_current";}else{$class=null;}
			$html=$html . "<li><a href=\"javascript:ContactTabs('$num')\" $class>$ligne</a></li>\n";
...
function CONTACT_INDEX(){
	if(!isset($_GET["uidUser"])){
		if(trim($_GET["employeeNumber"])==null){$_GET["employeeNumber"]=time()."-". rand(0,10000);}
		...
		<tr>
			<td valign='top'>
				<table style='width:100%'>
					<tr>
						<td valign='top'>
							<div style='border:1px solid #CCCCCC;width:61px;padding:2px'>" . imgtootltip($ct->img_identity,"{picture}","s_PopUp('edit.thumbnail.php?employeeNumber=$ct->employeeNumber',600,300);")."</div>
						</td>
						<td valign='top'><br>
							<table style='width:100%'>
								<tr>
									<td valign='top'>
										<input type='button' OnClick=\"javascript:CompleteName();\" value='{complete_name}&nbsp;&raquo;' 
		...
		<table style='width:100%;margin-left:10px'>
			<tr>
				...
				"mozillaSecondEmailCheck(event)")."
						...
						<td>" . imgtootltip("plus-24.png","{add}","mozillaSecondEmailAdd()")."</td>
					</tr>
				</table>
...
<table style='width:100%;margin-left:10px'>
			<tr>
				...
				<td valign='middle'>" . Field_text('labeledURI',"$ct->labeledURI",null,null,null,null,false,"EditContactCheck(event)")."</td>
			</tr>
		</table>
<hr>		
<div style='font-size:14px;font-weight:bold;margin-bottom:5px'>{work}</div>
		<table style='width:100%;margin-left:10px'>
			<tr>
				...
				<td valign='middle'>" . Field_text('businessRole',"$ct->businessRole",null,null,null,null,false,"EditContactCheck(event)",$businessRoleEnabled)."</td>
				<td class=legend valign='middle' nowrap>{working_title}:</td>
				<td valign='middle'>" . Field_text('title',"$ct->title",null,null,null,null,false,"EditContactCheck(event)")."</td>	
			</tr>
			<tr>			
				<td class=legend valign='middle' nowrap>{company}:</td>
				<td valign='middle'>" . Field_text('o',"$ct->o",null,null,null,null,false,"EditContactCheck(event)")."</td>
				<td class=legend valign='middle' nowrap>{department}:</td>
				<td valign='middle'>" . Field_text('ou',"$ct->ou",null,null,null,null,false,"EditContactCheck(event)")."</td>					
			</tr>	
			<tr>			
				<td class=legend valign='middle' nowrap>{managerName}:</td>
				<td valign='middle'>" . Field_text('managerName',"$ct->managerName",null,null,null,null,false,"EditContactCheck(event)",$managerNameEnabled)."</td>
				<td class=legend valign='middle' nowrap>{assistantName}:</td>
				<td valign='middle'>" . Field_text('assistantName',"$ct->assistantName",null,null,null,null,false,"EditContactCheck(event)",$assistantNameEnabled)."</td>					
			</tr>						
		</table>
<hr>		
...
<table style='width:100%;margin-left:10px'>
			<tr>
				...
				<td valign='middle'>" . Field_text('roomNumber',"$ct->roomNumber",null,null,null,null,false,"EditContactCheck(event)")."</td>
				<td class=legend valign='middle' nowrap>{birthDate}:</td>
				<td valign='middle'>" . Field_text('birthDate',"$ct->birthDate",null,null,null,null,false,"EditContactCheck(event)",$birthDateEnabled)."</td>	
			</tr>	
			<tr>
				<td class=legend valign='middle' nowrap>{spouseName}:</td>
				<td valign='middle'>" . Field_text('spouseName',"$ct->spouseName",null,null,null,null,false,"EditContactCheck(event)",$spouseNameEnabled)."</td>
				<td class=legend valign='middle' nowrap>{birthDate}:</td>
				<td valign='middle'>" . Field_text('anniversary',"$ct->anniversary",null,null,null,null,false,"EditContactCheck(event)",$anniversaryEnabled)."</td>	
			</tr>	
			<tr>
...
$form=RoundedLightWhite($form);
	...
	<table  style='width:100%'>
	<tr>
		<td align='right'><input type='button' OnClick=\"javascript:EditContact('FFM_CONTACT_PAGE');\" value='{edit}&nbsp;&raquo;&raquo;'></td>
	</tr>
	</table>";
	...
	$form="
		...
		<table style='width:100%;margin-left:2px'>
			...
			<tr>
				...
				<td valign='middle'>" . Field_text('l',"$ct->l",null,null,null,null,false,"EditContactCheck(event)")."</td>
			</tr>
				<td class=legend valign='middle' nowrap>{postalCode}:</td>
				<td valign='middle'>" . Field_text('postalCode',"$ct->postalCode",null,null,null,null,false,"EditContactCheck(event)")."</td>	
			</tr>
			</tr>
				<td class=legend valign='middle' nowrap>{state}:</td>
				<td valign='middle'>" . Field_text('st',"$ct->st",null,null,null,null,false,"EditContactCheck(event)")."</td>	
			</tr>	
			</tr>
				<td class=legend valign='middle' nowrap>{country}:</td>
				<td valign='middle'>" . Field_text('c',"$ct->c",null,null,null,null,false,"EditContactCheck(event)")."</td>	
			</tr>					
			</table>
			</td>
			</tr>
