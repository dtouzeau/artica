...
function mysql_settings($notitle=false){
	...
	if($notitle){
		$title="<H1>{mysql_account}</H1>";
	}
	
	
	
	$html=$title.RoundedLightWhite("
	
	<table style='width:100%'>
	
		<tr>
		<td align='right' nowrap class=legend>{mysqlserver}:</strong></td>
		<td align='left'>" . Field_text('mysqlserver',$servername,'width:100px',null,null,'')."</td>
		</tr>	
		<tr>
		<td align='right' nowrap class=legend>{mysqlroot}:</strong></td>
		<td align='left'>" . Field_text('mysqlroot',$rootm,'width:100px',null,null,'{mysqlroot_text}')."</td>
		</tr>
		<tr>
		<td align='right' nowrap class=legend>{mysqlpass}:</strong></td>
		<td align='left'>" . Field_password("mysqlpass",$pwd)."</td>
		</tr>
		<tr>
			<td colspan=2 align='right'><input type='button' OnClick=\"javascript:Loadjs('$page?script=mysql_save_account');\" value='{edit}&nbsp;&raquo;'></td>
		</tr>	
	</table>
	");	
...
function js_mysql_save_account(){
	...
	var mysqlroot=document.getElementById('mysqlroot').value;
	var mysqlpass=document.getElementById('mysqlpass').value;
