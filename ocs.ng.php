...
function params_web(){
	...
	$www=texttooltip("{website}:http://$ocswebservername:$OCSWebPort/ocsreports","{website}",
	"javascript:s_PopUpFull('http://$ocswebservername:$OCSWebPort/ocsreports',800,600)",null,0,"font-size:14px",1);
...
function popup_connected_search(){
	...
	while($ligne=mysql_fetch_array($results,MYSQL_ASSOC)){
		...
		if($uid==null){
			$js=null;
			$status=imgtootltip("status_warning.gif","{ocs_computer_is_not_in_ldap}<hr>$last","AddComputerFromOCS('{$ligne["MACADDR"]}')");
		}else{
	...
	$html=$html."
		<tr ". CellRollOver($js,$js_text).">
			...
			<td style='font-size:12px'>{$ligne["IPSRC"]}/{$ligne["IPADDRESS"]}</td>
		</tr>
