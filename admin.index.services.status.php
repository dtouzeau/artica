...
while (list ($num, $ligne) = each ($array) ){
		...
		$html[]= "<li><a href=\"$page?status=yes&section=$num&filterby={$_GET["filterby"]}\"><span>$ligne</span></li>\n";
	...
	if($application_installed<>1){
		...
		if($array["running"]==1){
			...
			if($service_disabled==0){
				...
				$status="{stopped}";
				if(strlen($service_cmd)>0){
					$js_service="Loadjs('StartStopServices.php?APP=$app&cmd=$service_cmd&action=start')";
