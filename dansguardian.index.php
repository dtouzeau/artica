...
function popup_tabs(){
	...
	while (list ($num, $ligne) = each ($array) ){
		...
		if($num=="rules"){
			$html[]=$tpl->_ENGINE_parse_body("<li><a href=\"$page?popup-rules=yes&switch={$_GET["switch"]}\"><span>$ligne</span></li>\n");
			continue;
		}
		
		if($num=="index"){
			$html[]=$tpl->_ENGINE_parse_body("<li><a href=\"$page?popup-index=yes&switch={$_GET["switch"]}\"><span>$ligne</span></li>\n");
			continue;
		}

		if($num=="authentication"){
			$html[]=$tpl->_ENGINE_parse_body("<li><a href=\"$page?popup-authentication=yes&switch={$_GET["switch"]}\"><span>$ligne</span></li>\n");
			continue;
		}			
...
SET_CACHED(__FILE__,__FUNCTION__,"dans",$html);
echo $html;
...
function main_rules_mainsettings(){
	...
	<td valign='top'>" . Field_array_Hash($groupmode,'groupmode',$dans->Main_array["groupmode"])."</td>
	<td valign='top'>" . help_icon('{groupmode_text}')."</td>
	</tr>		
	
	<td align='right' nowrap valign='top' class=legend>{naughtynesslimit}:</strong></td>
	<td valign='top'>" . Field_text('naughtynesslimit',$dans->Main_array["naughtynesslimit"],'width:50px')."</td>
	...
	<td valign='top'>" . Field_text('categorydisplaythreshold',$dans->Main_array["categorydisplaythreshold"],'width:50px')."</td>
	...
	<td valign='top'>" . Field_text('embeddedurlweight',$dans->Main_array["embeddedurlweight"],'width:50px')."</td>
	...
	<tr>
	<td align='right' nowrap valign='top' class=legend>{groupname}:</strong></td>
	<td valign='top'>" . Field_text('groupname',$dans->Main_array["groupname"],'width:150px')."</td>
...
function popup_authentication(){
	...
	if($squid->enable_squidguard==1){$groupipadd=Paragraphe("64-filter-computer.png","{filter_ip_group}",'{filter_ip_group_text}',"javascript:YahooWin(600,'$page?popup-group-ip=yes');");}
...
function popup_rules(){
	$rules=popup_rules_list();
	$html="
	<input type='hidden' id='dans_add_rule_text' value='{dans_add_rule_text}'>
	<div style='text-align:right;width:100%'>
	". button("{add_rule}","javascript:dansguardian_addrule('$hostname');")."
	</div>
	<div id='dans_rules_content'>
	</div>
	<div id='rules_lists' style='width:100%;height:300px;overflow:auto'>
	$rules
...
$categories=Paragraphe("64-categories.png","{categories}",'{banned_catgories_text}',
...
function ip_group_page(){
	...
	$html="
	<H1>{filter_ip_group}</H1>
	<p class=caption>{filter_ip_group_explain}</p>
	<table style='width:100%'>
	<tr>
		<td valign='top'>
		" . ip_group_rule_list() ."
...
while (list ($num, $val) = each ($rules) ){
		...
		$html=$html . "
		<tr>
			<td valign='top'><H3>$rulename</H3></td>
...
function ip_group_list($rule,$noecho=0){
	...
	$q=new mysql();
		...
		while($ligne=@mysql_fetch_array($results,MYSQL_ASSOC)){
			$num=$ligne["ID"];
			$val=$ligne["pattern"];
			$delete=imgtootltip("ed_delete.gif",'{delete}',"ip_group_delete('$rule','$num')");
			$html=$html . "
			<tr ". CellRollOver().">
				<td width=1%><img src='img/fw_bold.gif'></td>
				<td>$val</td>
...
function ip_group_js(){
	...
	function ip_group_start(){
		YahooWin3(400,'dansguardian.index.php?add-address-step1=yes','$title');
		}
	...
	function ip_group_wizard(){
		...
		YahooWin3(400,'dansguardian.index.php?add-address-step1=yes&selected-ip-family='+ip_family,'$title: '+ip_family);
	...
	function last_step(){
		...
		YahooWin3(400,'dansguardian.index.php?add-address-step1=yes&selected-ip-family=rule&pattern='+pattern,'$title: '+ip_family);
	}
...
$select=Field_array_Hash($arr,'ip_family',null);
$tpl=new templates();
...
$form="
	...
	<tr>
		<td class=legend>{select_ip_family}:</td>
		<td>$select</td>
	</tr>
	<tr><td colspan=2><hr></td></tr>
	<tr>	
		<td colspan=2 align='right'><input type='button' OnClick=\"javascript:ip_group_wizard()\" value='{next}&nbsp;&raquo;'></td>
	</table>";
...
$form="
	...
	<tr>
		...
		<td>" . Field_text('ip_address',null,'width:120px')."</td>
	</tr>
	<tr>	
	<tr><td colspan=2><hr></td></tr>
		<td><input type='button' OnClick=\"javascript:ip_group_start()\" value='&laquo;&nbsp;{back}'></td>
		<td align='right'><input type='button' OnClick=\"javascript:ip_group_single()\" value='{next}&nbsp;&raquo;'></td>
	</table>";	
...
$form="
	<p class=caption>Ex: <li>192.168.1.0</li> <li>255.255.255.0</li></p>
	<table class='table_form'>
	<tr>
	...
	<tr>
		...
		<td>" . Field_text('netmask',null,'width:120px')."</td>
	</tr>	
	<tr>	
	<tr><td colspan=2><hr></td></tr>
		<td><input type='button' OnClick=\"javascript:ip_group_start()\" value='&laquo;&nbsp;{back}'></td>
...
if($_GET["selected-ip-family"]=="rule"){
	...
	$html="
	<h1>{add_address}: {$_GET["selected-ip-family"]}</H1>
	<table style='width:100%'>
	<tr>
	<td valign='top'><img src='img/64-add-computer.png'></td>
	<td valign='top'>
		$form
	</td>
	</tr>
