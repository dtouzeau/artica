...
$q=new mysql();
	$sql="SELECT ID,pattern FROM dansguardian_files WHERE filename='bannediplist' AND RuleID=1 ORDER BY ID DESC";
...
function AddDansGuardianExceptionipList(){
	...
	if(!$dans->Add_bannediplist($_GET["AddDansGuardianExceptionipList"],$_GET["AddDansGuardianExceptionipList"],1)){
		echo $dans->error;
	}
