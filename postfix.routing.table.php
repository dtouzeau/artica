...
<table style='width:99%;padding:5px;border:2px solid #8E8785;' align='center' class=table_form>
	...
	while (list ($domain, $ligne) = each ($h) ){
		...
		<td style='font-size:13px'><code $edit>$domain</strong></code></td>
		<td width=1%><img src='img/fw_bold.gif'></td>
		<td align='center' style='font-size:12px' $edit>{$array[0]}</td>
		<td $edit style='font-size:13px'><code>{$array[1]}</code></td>
		<td align='center' style='font-size:13px'><code>{$array[2]}</code></td>
		<td align='center' style='font-size:13px'><code>{$array[3]}</code></td>
...
function PostfixAddRoutingTableSave(){
	...
	if($_GET["domain"]==null){
		echo $tpl->_ENGINE_parse_body("{error_no_domain_specified}");
		exit;
		}	
...
$form="<h1>{relayhost}</H1>
	<table style='width:100%'>
	<tr>
		<td valign='top'>$otherisp
		</td>
		<td valign='top'>
			...
			<div id='relayhostdiv'>
					...
					<input type='hidden' name='relayhostSave' value='yes'>
						<table style='width:100%'>
						<td align='right' nowrap class=legend>{relay_address}:</strong></td>
						<td style='font-size:12px'>" . Field_text('relay_address',$relayT[1]) . "</td>	
						</tr>
						</tr>
						<td align='right' nowrap class=legend>{smtp_port}:</strong></td>
						<td style='font-size:12px'>" . Field_text('relay_port',$relayT[2]) . "</td>	
						</tr>	
						<tr>
