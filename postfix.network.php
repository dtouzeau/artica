...
function BindInterfaceForm($noecho=0){
	...
	$fieldIP=Field_array_Hash($array_IP,"inet_interface_select",null);
...
$html="<div id='BindInterfaceForm'>
	...
	<tr>
		...
		<td align='left'>" . Field_text('inet_interface_add',null,'width:80%',null,null,'{inet_interfaces_text}') ."</td>
	</tr>
	<tr>
	...
	<script>
		...
		function PostfixDeleteInterface(num){
			...
			document.getElementById('BindInterfaceForm').innerHTML=\"<center style='width:100%'><img src='img/wait_verybig.gif'></center>\";
			XHR.sendAndLoad(Working_page, 'GET',x_ReloadInterface);
			}			
...
function NetworkInterfacesForm($noecho=0){
	...
	$page=CurrentPageName();
$html="
<span style='font-size:12px;font-weight:bold'>{mynetworks_title}</span>
	<table style='width:90%' align='center'>
	<tr>
...
function BindInterfaceTable(){
	...
	while (list ($num, $val) = each ($table) ){
		...
		$html=$html . "
		<tr>
			<td width=1%><img src='img/icon_newest_reply.gif'></td>
			<td style='font-size:16px'>$val</td>
			<td  width=1%>" . imgtootltip('x.gif','{delete} {inet_interface}',"PostfixDeleteInterface($num)") ."</td>
		</tr>";
...
function mynetworks_table(){
	...
	while (list ($num, $val) = each ($main->array_mynetworks) ){
		...
		<tr ". CellRollOver().">
			...
			<td  width=1%>" . imgtootltip('x.gif','{delete} {network}',"PostFixDeleteMyNetwork($num)") ."</td>
		</tr>";
		}
	}
