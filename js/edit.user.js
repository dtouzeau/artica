...
function UserADD(){
	...
	if(document.getElementById('adduser_ajax_newfrm')){document.getElementById('adduser_ajax_newfrm').innerHTML='<center><img src="img/wait-clock.gif"></center>';}
	if(document.getElementById('bglego')){document.getElementById('bglego').src='img/wait-clock.gif';}
	if(document.getElementById('member_add_to_wait')){document.getElementById('member_add_to_wait').innerHTML='<center><img src="img/wait-clock.gif"></center>';}
	
	
	
	XHR.sendAndLoad('domains.edit.user.php', 'GET',X_UserADD);	
	}
...
function LoadGroupForMember(ou,uid){
	YahooWin4('400','domains.edit.user.php?USER_GROUP_ADD=yes&ou='+ou+'&uid='+uid,'Groups');
	}
	
var x_AddPopUpGroupAjaxMode= function (obj) {
	LoadAjax('POPUP_MEMBER_GROUP_ID','domains.edit.user.php'+'?POPUP_MEMBER_GROUP_ID=yes&ajaxmode=yes&ou='+m_ou+'&userid='+m_userid);
	
}	
	
function AddAjaxPopUpGroupV2(ou,uid,gid,gidname){
	var text=document.getElementById("ADD_USER_GROUP_ASK").value+'\n\nuid:'+uid+'\nGroup:'+gid+'\nName:'+gidname;
	if(confirm(text)){
		m_userid=uid;
		group=gid;
		m_ou=ou;
		...
		XHR.appendData('AddMemberGroup',group);
		document.getElementById('POPUP_MEMBER_GROUP_ID').innerHTML='<center><img src="img/wait_verybig.gif"></center>';
		XHR.sendAndLoad('domains.edit.user.php', 'GET',x_AddPopUpGroupAjaxMode);
		}
...
function ComputerRefresh(){
	...
	YahooUser(670,'domains.edit.user.php?userid='+computer+'$&ajaxmode=yes',computer);
	}
