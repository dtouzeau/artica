...
function Browse(){
	LoadWinORG(740,'$page?main_disks_discover=yes&t={$_GET["t"]}&homeDirectory={$_GET["homeDirectory"]}&no-shares={$_GET["no-shares"]}&field={$_GET["field"]}&protocol={$_GET["protocol"]}&no-hidden={$_GET["no-hidden"]}');
...
function hidden_disk_popup(){
	...
	<p class=caption>{add_invisible_disk_text}</p>
	<table style='width:100%'>
	<tr>
...
function TreeRightInfos(){
	...
	if($users->SAMBA_INSTALLED){
		...
		if($smb->main_shared_folders[$path]<>null){
			$txt="<tr><td colspan=2><p class=caption><b>$path</b>&nbsp;({$smb->main_shared_folders[$path]})&nbsp;:{FOLDER_IS_SHARED}</p></td></tr>";
	...
	<H$h>&laquo;$f&raquo; {directory}</H$h>
