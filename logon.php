...
if(!function_exists('ldap_connect')){
	...
	echo "
	<html><head></head><body>
	<div style='border:3px solid red;font-size:22px;color:red;padding:5px;margin:100px;width:70%'>
...
echo "
<div style='margin:200px;padding:10px;border:2px solid red'><center>
<H1>
<error>Artica-postfix need php and <strong>posix &laquo;posix_getuid()&raquo;</strong>  function <BR>... please try to install php-posix</error>
<div style='color:red;font-size:13px'>Unable to stat posix_getuid function</div>
</H1></div>";exit;}	
...
function logonForm(){
	...
	$field_lang=Field_array_Hash($lang,'lang',$defaultlanguage);	
...
<form name='logon_{$_SERVER["SERVER_NAME"]}' method='POST'>
				<table style='width:50%;border:3px solid #005447;margin:5px;padding:5px;'>
				<tr>
				<td align='right' class=legend style='font-size:13px'><strong>{username}:</strong></td>
				<td><input type='text' id='artica_username' name='artica_username' value='' 
				...
				value=''style='border:1px solid black;width:100%;font-size:13px'
				OnkeyPress=\"javascript:SendLogon(event)\"></td>
				</tr>
				<tr>
